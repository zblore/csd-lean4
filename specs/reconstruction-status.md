...
>    `RecordLayer/GlobalBasin.lean` + `GlobalRecordClosure.lean` put the record layer on a
...
|---|---|
| **A1** compact Kähler ontic surface | **Partial.** Compactness and the *pointwise* Kähler compatibility core are represented (`IsFubiniStudyKahler`, `fubiniStudy_pointwise_kahler_compatibility`); full manifold exterior calculus (`dω=0`, `ωⁿ/n! = μ_FS`) is formalised (2026-09-08; the Kähler predicate 2026-09-10, `fsForm_isKahler`) and wired to the sectors (`LF4/SectorManifold.lean`, W1). ⚠️ **Separately (2026-07-30): the *fibred* Σ's A1 status is a dimension-parity question, not a tooling one** — `ℂℙⁿ⁻¹ × ℝ` (`FibredSigma`) and `ℂℙⁿ⁻¹ × AddCircle 1` (`CircleFibre`) both have real dimension `2n-1`, **odd**, so neither admits *any* symplectic, hence Kähler, structure. Only `KSigma = ℂℙⁿ⁻¹ × T²` (dimension `2n`) can. See §2a. |
| **A2** Hamiltonian ontic dynamics — ⚠️ *the witness that carries it pays a trilemma horn; see `SigmaLayer/FiniteQMClosure.lean`, "Which trilemma horn this closure pays" (**seams**: unitary interaction, exact Born, a.e.-total readout), and `docs/TOUR.md`, "The three priced witnesses"* | **Formalisable half DISCHARGED 2026-08-02** (`RecordLayer/HamiltonianSignature.lean`): the witness flow carries every Hamiltonian **signature** a measure space can express — a canonical conserved energy (`onticEnergy`, ray-well-defined, conserved under the flow: `onticEnergy_flow_invariant`), which is *simultaneously* A5's exactly-projectable `h` (`onticEnergy_epsProjectable` — the A2/A5 junction); the commuting phase torus preserving every moment-map coordinate (`momentMap_phaseDiag_invariant`, `phaseDiag_comm`); plus the already-proved Liouville property and group laws, with the fibre translations measure-preserving (`ShearWitness`). **The vector-field equation `X_H = ω⁻¹dH` itself is §2a-scoped** (symplectic form + exterior derivative; verified a tooling gap). The witness uses the projected `e^{-itH}` lift. ⚠️ *Correction 2026-08-02 (external review, machine-checked same day):* the **measurement-interval** propagators (`shearEvolve`, `swapEvolve`) are **provably not continuous** (`shearEvolve_not_continuous`, `RecordLayer/ShearDiscontinuity.lean`) — they are measurable, measure-preserving, *piecewise* maps, so they are **not** time slices of any continuous flow, and the Hamiltonian-generation question is **reopened**, not Mathlib-scoped (`BACKLOG.md`, decision pending: corridor regularisation vs piecewise-Hamiltonian classification). The *isolated* dynamics (this row's flow) is untouched by the correction — `e^{-itH}` on rays is continuous; the discontinuity enters only with the de-isolating measurement interaction, exactly where `no_everywhere_correlation` proves some seam set is forced. *Resolution 2026-08-02 (user decision):* the piecewise classification landed (`RecordLayer/PiecewiseHamiltonian.lean`) — each basin cylinder carries an explicit continuous rigid translation, the seam set is null (`seam_null`). ⚠️ *Second correction, same day (second external review):* **(flux)** those translations are symplectic but **not globally Hamiltonian** on the compact torus (`ι_Xω = a·dp` closed-not-exact; `∮dp ≠ 0`), so the classification's correct name is **piecewise rigid symplectic translation**, not "piecewise Hamiltonian" — the `hᵢ = aᵢ·p_R` reading is withdrawn; **(overread)** `no_everywhere_correlation` forces an exceptional set, not discontinuity — a continuous globally-Hamiltonian propagator with seams mapped to transition states remains open. The Hamiltonian-generation question is **REOPENED** with the recommended compact-Kähler-pointer route `ℂℙ^K` (`BACKLOG.md`); *route confirmed 2026-08-03 (author decision): the smooth-Hamiltonian architecture of Paper C A2 / TN6 is retained, and the `ℂℙ^K` pointer witness is the obligation — execution plan `pointer-witness-plan.md`.* **★ DELIVERED 2026-08-03 (all five bricks, `pointer-witness-plan.md`):** `smoothWitnessClosure` — one witness carrying the measurement protocol (two-time law = exponential group property), **joint time–state continuity** (`continuous_pointerRampedEvolve`; contrast `shearEvolve_not_continuous`), Liouville preservation, a positive-measure ready state (no Dirac calibration posit — the preparation conditions on the positive-measure ready region), record creation with the ontic sector selecting the outcome, structural persistence, and the ε-Born sandwich `rⱼ − 2ε ≤ sector ≤ rⱼ + 2(N−1)ε`; and ★ `rampedU_schrodinger` — on the interaction window the propagator satisfies the **Schrödinger equation** with the explicit **Hermitian** generator `H_eff = (π/2)•couplingH w` (single-plane exponential identification `pointerRot_eq_exp` by ODE uniqueness). **The Hamiltonian-generation sub-question is discharged at the formalisable level**: the flux obstruction is gone (projective pointer, `H¹(ℂℙ^K) = 0`, generator exhibited); the symplectic/moment-map *reading* remains the same §2a-scoped prose boundary as A1/A3 (Mathlib has no symplectic API); exact records/Born are traded for the stated `ε` per `no_everywhere_correlation` — the exact-record piecewise witnesses stand alongside. ⚠️ **Corrected 2026-08-04 (independent Opus review of HEAD).* — this row said "**the** other horn", a two-horn framing, and this file (which `README.md` sends readers to for the axiom-level audit) contained **no mention of the third horn at all**. The fork is a **trilemma**: seams (piecewise), `ε`-Born (smooth witness), or Dirac calibration (`nullSeamClosure`, `RecordLayer/NullSeamWitness.lean` — continuous, measure-preserving, exact Born off a two-point seam). Whether a fourth combination is impossible is open, not claimed.* ⚠️ *Boundary sharpened 2026-08-03 (fourth external review, verified both ways):* the delivered generation is **fibrewise** — the arena propagator's generator is vertical `(0, V_Q)`, and for the joint scalar `𝓗(x,q) = μ_{H_eff(w(x))}(q)` one has `ι_Vω ≠ d𝓗` on the ε-collars where the weights vary (off the collars the weights are locally constant and the flow is genuinely locally Hamiltonian; the defect shares the Born error's `O(ε)` budget). The suppressed horizontal component is register back-reaction — `pointerEvolve_base_marginal_unchanged` states it as a theorem. The accurate name is **continuous fibrewise-Schrödinger witness**: "smooth horn" names the ε-corridor architecture, and the weight/ramp ingredients are Lipschitz, proved `Continuous`, not `C^∞` (the `Real.smoothTransition` upgrade **landed 2026-08-03**, `RecordLayer/PointerSmoothProfile.lean`; substituting those profiles into the protocol stack is the part still outstanding — `BACKLOG.md` B1). The formalisable-level discharge stands **as scoped**; the full back-reacting joint-arena flow — where the register moves mid-stroke and exact moment transport no longer applies — is the recorded **research** row in `BACKLOG.md`, a frontier, not a silent claim. |
| **A3** smooth many-to-one projection | **Discharged 2026-09-11 (Q33).** The arena `ℂℙⁿ × T²` is an analytic manifold (`CSD.LF4.ksigma_isManifold`; `AddCircle` made a manifold in `Instances/AddCircle.lean`) and `π = Prod.fst` is `C^ω` (`manyToOneSetup_pi_contMDiff`). Lean's proofs use only measurability; the witness is genuinely many-to-one (the `T²` fibre). |
...
| **A5** quantum-effective Hamiltonians (projectability) — ⚠️ *the unitary class is a separate posit (`specs/POSITS.md` Posit 6, review Part II Posit 4): `projectedFlow_eq_unitary_family` takes `ProjUnitary` as a hypothesis and selects the family, so the corpus proves "unitary ⇒ Schrödinger" (`CL-063`) and posits "unitary". Motivated by no-signalling (Gisin 1990), not derived from it; see `LF4/ProjectedDynamics.lean`* | **Approximate case FORMALISED 2026-08-02** (`RecordLayer/ApproxProjectability.lean` + the Duhamel bound): `EpsProjectable` (oscillation form; the derivative form is the §2a-scoped manifold refinement, substitution stated), `epsProjectable_zero_iff` (exact case ⇔ `ε = 0`, an iff), and the **shadowing theorem** — `‖H−H₀‖ ≤ ε`, `|t| ≤ T` ⇒ sector dynamics tracks true dynamics within `ε·T`. The exact fibre-invariant case `H = h∘π` was already formalised (projected flow closes, `e^{-itH}` on rays). ⚠️ Residue: the shadowing is Hilbert-side; an ontic Hamiltonian *generating* the flow is A2's row. |
| **A6** composites + marginal stability | **Non-factorisation PROVED 2026-08-02** (`RecordLayer/OnticComposite.lean`): the Segre embedding `([u],[v]) ↦ [u⊗v]` is **injective but not surjective** at every dimension pair ≥ 2×2 (Bell-type witness), so `Σ_AB ⊋ image(Σ_A × Σ_B)` — the ontic composite strictly exceeds the product of sectors, as a theorem. Operational tensor structure, reduced states and no-signalling already existed (`compositeTensorEquiv`, `tensorSector_no_signalling`). **Steps 2–3 DONE 2026-08-02** (`RecordLayer/OnticMarginals.lean`): ontic reduction maps (`rayDensity` unit-trace + `reduceA`/`reduceB` partial traces), and **marginal stability under local flows** — `reduceB_local_flow_invariant`: the Schrödinger flow of any `A`-Hamiltonian leaves the `B`-marginal fixed at every time (ontic no-signalling), with the `A`-marginal evolving by conjugation. **Every A6 clause the corpus can express is now a theorem.** ~~Remaining: step 4 (dynamical no-signalling through the measurement layer — optional capstone)~~ **Step 4 DONE 2026-08-03** (`RecordLayer/LocalLuders.lean` → `LocalLudersBasis.lean`): a local measurement IS a block-degenerate measurement (`toComposite_blockProj`, definitional under the index bridge), and the Born-weighted mixture of post-measurement local marginals equals the pre-measurement marginal **in every measurement basis** (`reduceA_localLudersOn_mixture`) — the distant outcome *and the basis choice* are dynamically invisible, with the post-states exactly those the join witness delivers (`BlockLudersObligation`). The eraser rides the same machinery as a process (`EraserDynamics`, `EraserSequential` — including `sequential_no_revival`: records are statistically irreversible). A6-as-philosophy (`Σ_AB` primitive) is not a formalisation target. **Generation premise in record vocabulary 2026-09-02** (`SigmaLayer/TensorTomography.lean`): `hgen` of the "why `⊗`" reconstruction ⟺ record-level local tomography (`recordLocallyTomographic_iff_adjoin_eq_top`); the surviving posit is boundary `R-017`. |
...
`RecordLayer/GlobalBasin.lean` defines `ContextField` (a measurable simplex-valued rate *field* on the
...
`RecordLayer/GlobalRecordClosure.lean` carries the five record-layer facts on it over
...
* **A3, smoothness of `π`** — ✅ **DISCHARGED 2026-09-11 (Q33, same day it was re-classified).**
  `AddCircle T` is an analytic manifold (`Mathlib/Geometry/Manifold/Instances/AddCircle.lean`: a
  charted space and all its groupoids transport along a homeomorphism, applied to
  `AddCircle.homeomorphCircle`), so the arena `KSigma = ℂℙⁿ × T²` is one (`CSD.LF4.ksigma_isManifold`)
  and `π = Prod.fst` is `C^ω` (`manyToOneSetup_pi_contMDiff`, `LF4/SectorManifold.lean`). Lean's
  proofs still use only measurability, and the witness is genuinely many-to-one.
...
Two modules landed on 2026-07-29/30. `RecordLayer/CircleFibre.lean` puts the active Born partition on
a **compact** fibre `AddCircle 1`, with Haar a genuine probability measure and cells carrying exactly
the Born weights; `RecordLayer/CircleRecord.lean` gives the compact counterpart of the P5 record
...
**Net corrected 2026-08-02 (evening): A2's Hamiltonian-generation sub-question is REOPENED (torus-flux, second external review — see the A2 row); the rest of the net line stands.** **Net for "finite QM from Σ" (updated 2026-08-02): three rows permanently scoped (A1-exterior *on
...
equation is scoped), and **A6's expressible content proved 2026-08-02** (non-factorisation via the
Segre embedding; reduction maps; marginal stability under local flows). **The A1–A7 map has no
unscoped open rows left**: every row is proved, discharged, closed-as-posit, or explicitly scoped
with a verified reason. The frontier is now the recorded L-scale items (degenerate Lüders via the
...
| Link | Claim | Status |
|---|---|---|
| L1 | Kähler geometry ⇒ sector fields | DISCHARGED 2026-09-10 (W1; was PARTIAL) — volume forced; 2-form's **pointwise** compatibility core is the concrete field `kahler_pointwise : IsFubiniStudyKahler` (F-04 tightening 2026-08-06); manifold closedness `dω=0` / `ω^{∧n}/n!=μ_FS` formalised 2026-09-08 and wired 2026-09-10 (`LF4/SectorManifold.lean`) |
...
* **The record layer (MD-1) — the KINEMATIC interface is BUILT (2026-07-25), formalized end-to-end;
  ...
  the model space **Σ = base × fibre** (`RecordLayer/FibredSigma.lean`): the **base** `CPN n` is
  the *epistemic* projective point (pinned to `[ψ]` for a sharp prep), the **fibre** carries the *ontic*
  record coordinate. The measurement is `context + unknown microstate → record` (`RecordLayer/Measurement.lean`):
  ...
  (`RecordLayer/MomentMapRace.lean`, `bornRate_eq_momentMap` — read off the context, not injected; the choice of rate field is forced by torus *generation* (`RecordLayer/CellLawForced.lean`, `torusGenerated_eq_momentMap`), not by invariance),
  and
  ...
  in **is definitionally the record-layer event**. Arbitrary observables via `RecordLayer/BasisMeasurement.lean`.
...
* **A6 "why ⊗"** — SUFFICIENCY + NECESSITY both proved (`SigmaLayer/TensorSolved.lean`
  ...
  from nothing (it singles out quantum `⊗`) — since 2026-09-02 stated in record vocabulary and proved
  equivalent to the generation premise (`SigmaLayer/TensorTomography.lean`
