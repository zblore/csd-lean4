> **📦 ARCHIVED 2026-08-02.** This is the README as it stood before the v0.6–v1.1 arc
> (context-fixed basins, the dynamical measurement layer, the unified arena, the
> degenerate-Lüders join resolution). Several of its status claims are stale — kept
> verbatim as the historical record per the repo's dated-corrections convention. The
> live README is [`../../README.md`](../../README.md).
...
The Kähler instance is `KSigma M = CPN M × KTorus = ℂℙ^{M-1} × (AddCircle 1 × AddCircle 1)` with `kMuL = fsMeasure p₀ ⊗ vol_T²`. The preparation measure for §14.2 is `(Measure.dirac p₀) ⊗ vol_T²` (Dirac on the base ray × Haar on the fibre).
...
- `SectorData` (LF2) is the abstract layer; the compact Kähler instance is `KSigma M` defined in `LF4/KahlerInstance.lean`, with `fsMeasure p₀` (Dirac at `p₀ : CPN M`) and `vol_T²` (Haar on the flat torus). Fibre arcs are subsets of `AddCircle 1` carved via `equivIoc 1 0`.
...
| `fs_born_volume_ratio_qubit` | `LF4/BornFS.lean` | Born weight = genuine `fsMeasure` volume ratio on `Σ=ℂℙ¹`, modulo `h_uniform`. | none |
