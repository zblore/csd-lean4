...
Status: **COMPLETE 2026-08-18** (scoped 2026-08-17, all three rows closed within
two days: CV-24 and CV-23a same-day, CV-23b/c 2026-08-17, CV-25 scoping
2026-08-18; BACKLOG row **Q21**; rows CV-23..CV-26 in
...
| CV-25 | **Channel-level RG — the scoping session ONLY** (Q11 mold: a doc, not theorems). Map the CPTP coarse-graining candidates (partial trace over decimated modes vs `compressCfg`-conjugation Kraus), fix the norm, and determine whether the existing Duhamel/price ladder already supplies the error budget `ε(λ, τ, distance)`; name the first brick or return the row to unqueued. The Stage-4 no-go is the floor any statement must respect: exact unitary matching is impossible for support-spreading drives, so the target is approximate channel matching with a priced defect. | Stage 4 record, K2 channels | **Research, GATED** | **SCOPING COMPLETE 2026-08-18** ([`channel-rg-scoping.md`](channel-rg-scoping.md)): budget provable, `ε = 2n\|τ\|\|λ\|C_v`; route = mode tracing; first brick **CR-1** named; arc queued on decision as CV-26 |

## CV-23b / CV-23c construction notes (recorded 2026-08-17, so the next session starts warm)

Written at the close of the CV-23a/CV-24 session, in the D3b spirit: at writing time
nothing below was proved; it was the worked design so the clean-context session
re-derives nothing. **CV-23b executed 2026-08-17** (`CV/Wick.lean`) — the session
followed these notes as written (kernel two-factor form, the cross-pairing exponent
identity, the brick list, the clustering-plus-collapse route), with zero re-derivation.
The CV-23c notes remain live for the gate.
...
* **CV-25 gate:** one focused scoping pass. If no statement with a *provable*
  ...
  the Stage-4 record is not to be re-litigated by optimism. **OUTCOME
  2026-08-18: the criterion is MET** — a provable budget emerges
