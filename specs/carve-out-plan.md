...
|---|---|---|---|
| **L0** | preparation measure `μL` = Kähler / Liouville volume | §2.3, §2.4 | partial: `KahlerInstance` uses `fsMeasure ⊗ vol_T²`, but `Φ = id` (debt **D1**) |
...
| **L2** | outcome region `Σᵢ` generated by SA dynamics, not posited | §9.5 — ⚠️ *split 2026-09-05, do not cite as one open target* | the carving stands in for this; debt **G3b / A5**. The three parts have different status: *which* interaction an apparatus realises is a permanent boundary (`R-015`), the chart→arena generation statement is open mathematics (`R-016`), and deriving the rate field is open foundations (`specs/POSITS.md` Posit 1's discharge condition). `ShearDeIsolation.lean` discharges `basin_rate` from a **constructed** propagator — a witness, not a derivation |
...
- **Slice 2 — reduction DONE 2026-05-29, analytic core OPEN**
  ...
  `Φ∗ fsMeasure = uniform_Δ`, which would lift `born_eq_volume_ratio`
  from a Lebesgue ratio on the polytope to a genuine μ_FS ratio on the ontic `Σ`.
  Key finding: the project's `fsMeasure` is the **Haar-on-U(N)
  ...
  ```
  Φ∗ fsMeasure = uniform_Δ  ⟺  squared-moduli of a Haar-random unit column ~ Dirichlet(1ᴺ).
  ```
  ...
  **Option C — DONE 2026-05-29** (`CsdLean4/LF4/BornFS.lean`, axiom-clean,
  ...
  - `fs_born_volume_ratio_qubit` (headline): for unit `ψ : ℂ²`, given
    `(fun p => momentMap p 0)∗ fsMeasure = uniform[0,1]`, then
    `fsMeasure {p | momentMap p 0 ≤ momentMap [ψ] 0} = ‖⟨e₀,ψ⟩‖²`. The
    ...
    `fsMeasure` on the ontic `Σ = ℂℙ¹`; no carving, no Busch.
  ...
  **Option B — the remaining discharge (the axiom's proof; deferred — user call
  ...
  `(momentMap · 0)∗ fsMeasure = uniform[0,1]` = "`|U₀₀|²` is
...
Haar pushforward `fsMeasure_pos_of_isOpen`), contradicting the zero-one law.
