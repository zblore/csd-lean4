...
|---|---|---|---|
| **L1** | Kähler geometry `⇒` the sector's fields | **PARTIAL (2026-07-07 fix C5; pointwise Kähler core DISCHARGED 2026-07-19; fields made CONCRETE 2026-08-06, F-04): both geometry fields are concrete structure fields any consumer can unpack; only the manifold residual remains** | `liouville_isProbability` carries the formalizable core — `μ_FS` is a *normalized* volume (probability measure), now an instance — exposed by `unitaryFlowSetup_liouville_isProbability` (`LF4/NonTrivialSetup.lean`). The Kähler condition is the concrete field `kahler_pointwise : IsFubiniStudyKahler N` — the pointwise Fubini–Study Kähler-compatibility triple (`g = re⟪·,·⟫`, `ω = im⟪·,·⟫`, `J = i•·`, with `J² = -1`, `ω = g∘J`, `g = ω∘J`, `ω` a `(1,1)`-form, `ω u (Ju) = ‖u‖²`), PROVED axiom-free (`CSD.LF4.isFubiniStudyKahler` / `Kahler.fubiniStudy_pointwise_kahler_compatibility`, `Mathlib/Analysis/InnerProductSpace/KahlerForm.lean`). The **flat** closedness `dω = 0` is now PROVED on the tangent model (2026-08-06, `Kahler.extDeriv_fundamentalFormAlt` in `Mathlib/Analysis/InnerProductSpace/KahlerClosed.lean`, via the generic `extDeriv_const`); what stays unformalizable is the **manifold** residual — closedness and the top-power identity `ω^{∧(N-1)}/(N-1)! = μ_FS` ON `ℂℙ^{N-1}` itself — pending Mathlib manifold-form API. |
...
- **L1** — the Kähler-geometry fields now carry genuine formalizable content
  ...
  since the 2026-08-06 F-04 tightening). Only the
