...
predated fixes C1–C7 and the Σ-layer and contradicted the CONNECTED rows below.)**
...
ontic model `(Σ, μL, Φ, π)` — the unified capstone `unified_projectiveSector_capstone`
...
the sector is **POSITED** — Σ is the floor, and **deriving it is a non-question** (the
...
**complete the reconstruction of QM from Σ and Ω-regions**; the record layer that does this is now
...
manifold residual (`dω = 0`, top-power volume identity) was an interpretive posit until 2026-09-10 (W1, `LF4/SectorManifold.lean`: L1 DISCHARGED)
...
|---|---|---|---|
| **L1** | Kähler geometry `⇒` the sector's fields | **DISCHARGED 2026-09-10 (W1). History: PARTIAL (2026-07-07 fix C5; pointwise Kähler core DISCHARGED 2026-07-19; fields made CONCRETE 2026-08-06, F-04): both geometry fields are concrete structure fields any consumer can unpack; the manifold residual is now proved and wired** | `liouville_isProbability` carries the formalizable core — `μ_FS` is a *normalized* volume (probability measure), now an instance — exposed by `unitaryFlowSetup_liouville_isProbability` (`LF4/NonTrivialSetup.lean`). The Kähler condition is the concrete field `kahler_pointwise : IsFubiniStudyKahler N` — the pointwise Fubini–Study Kähler-compatibility triple (`g = re⟪·,·⟫`, `ω = im⟪·,·⟫`, `J = i•·`, with `J² = -1`, `ω = g∘J`, `g = ω∘J`, `ω` a `(1,1)`-form, `ω u (Ju) = ‖u‖²`), PROVED axiom-free (`CSD.LF4.isFubiniStudyKahler` / `isFubiniStudyKahler`, `Mathlib/Analysis/InnerProductSpace/KahlerForm.lean`). The **flat** closedness `dω = 0` is now PROVED on the tangent model (2026-08-06, `Kahler.extDeriv_fundamentalFormAlt` in `Mathlib/Analysis/InnerProductSpace/KahlerClosed.lean`, via the generic `extDeriv_const`); The **manifold** residual — closedness and the top-power identity `ω^{∧(N-1)}/(N-1)! = μ_FS` ON `ℂℙ^{N-1}` itself — is PROVED (2026-09-07/08: `Projectivization.fsForm_mextDeriv`, `fsForm_isSymplectic`, `fsVolume_eq_smul_fsMeasure`; Kähler 2026-09-10: `fsForm_isKahler`) and WIRED to the sectors (2026-09-10, `LF4/SectorManifold.lean`: `unitaryFlowSetup_liouvilleMeasure_eq_fsVolumeNormalized`, `fsVolumeNormalized_isForcedKahlerVolume`, `unitaryFlowSetup_isKahler_liouville`, `manyToOneSetup_isKahler_liouville`). The structure's fields are unchanged; on the `ℂℙⁿ` instances they are the standard objects. |
...
| **L7** | Born weights derived FROM the deterministic flow | **N/A — deriving Σ is a non-question; but *constraining* Σ is real work** | "Derive the sector/Born from the flow" is retired: **Σ is the floor; deriving it is a non-question.** Born *arises from* the Ω-region volume ratios + typicality — that IS the reconstruction. The theorems once filed here are **constraint work on Σ** (Σ is hidden, so pin it down as tightly as possible, from above): `SigmaLayer/LocalisedTypicality.lean` shows the full `U(N)` symmetry *forces* `μ_FS` (Paper B's uniqueness, in Lean) — Σ's measure is not arbitrary; `SigmaLayer/SectorPostulateNoGo.lean` (`flow_admits_invariant_ne_fubiniStudy`) records only that a single *epistemic* flow does not time-average to `μ_FS` — expected, irrelevant to CSD's mechanism (typicality is repeated-preparation ignorance over `Ω₀`). Legitimate constraint work, not a *research frontier*. |
...
| **L9** | ONE ontic model carries isolated dynamics AND de-isolating measurement AND records AND Born AND state update | **CONNECTED (2026-07-17: assembled into one tiered closure `FiniteQMClosure`)** | `unified_projectiveSector_capstone` (`SigmaLayer/UnifiedMeasurement.lean`) BUNDLES the dynamics + measurement CORE on the SAME `(Σ = ℂℙ^{M}×T², μL = μFS ⊗ vol, π = Prod.fst)`: the ISOLATED dynamics is the genuine `exp(-itH)` Hamiltonian flow (`productDynamics`, measure-preserving + Schrödinger-projectable + `π_*μL = μFS`) AND a `DeisolationModel` over that same dynamics realises the de-isolation MEASUREMENT (`(measurementFlow p, θ)`, measure-preserving, a.e.-defined pointer readout `vnDeisolationModel_ae_total`, record establishment). All the remaining pieces are now ASSEMBLED with it INTO one tiered record **`FiniteQMClosure` / `unifiedFiniteQMClosure`** (`SigmaLayer/FiniteQMClosure.lean`): time-physical records — INSTANTIATED on the unified model (`unifiedFlowedSemantics` = `flowedSemantics` over the isolated `productDynamics` flow; `unified_records_persistence`: Born weight conserved + flow-covariant; `unifiedFlowedSemantics_zero`: the static `vnRecordSemanticsProd` is the `t=0` slice), #5; post-outcome preparation with proven nonzero measure (`PostMeasurement.appendFact`); Born-frequency convergence — stated ON the model (`unified_born_frequency`: i.i.d. trials of `productDynamics.muL`, frequency in `π⁻¹(bornRegion i)` → `‖⟨eᵢ,ψ⟩‖²`, for EVERY unit `ψ` — `hpos` retired via the `_uncond` engine), #2; and the state update = record conditioning = Lüders update — GENUINELY proved (`ConditioningLuders.conditioning_luders_effect_equivalence`: the ontic record-conditioning and the Lüders update give the same conditional prediction for every pointer-basis effect, via the proved weight agreement `onticRegion_measure_eq_born`; the earlier `ConditioningLink.luders_record_conditioning_correspondence` only conjoined the two Bayesian halves and ASSERTED their agreement); and mixed-state Born (`MixedOntic.mixed_ontic_born_weight` = the `mixed_born` field, #8 C: the classical mixture over `ρ`'s spectral ensemble of ontic Born-region measures reproduces `Tr(ρ Eᵢ)`, so the model carries mixed-state Born content, not only the pure `ψ`). `FiniteQMClosure` carries these eleven as fields (the tenth/eleventh being mixed-state Born as a weight `mixed_born` and as an a.s. frequency `mixed_born_frequency` / `unified_mixed_born_frequency`, `SigmaLayer/MixedFrequency.lean`: a two-stage mixture LLN converging to `Tr(ρ Eᵢ)`), each discharged by its source lemma; the Choice-A posit, QM adapters, and open residue are documented in the module header (no field is `sorry`) — so the tiers stay honest. AxiomAudit-pinned. Consumes the posited sector — Σ is the floor (deriving it is a non-question); the record layer is now built; the frontier is now the extensions, not this. **Addendum 2026-08-02 — L9 SUPERSEDED BY A STRONGER WITNESS:** `unifiedArenaClosure` (`RecordLayer/UnifiedArena.lean`) carries the whole reconstruction on **one arena with one Liouville measure family** — the lifted `exp(-itH)` and the record-creating `swapEvolve` preserve the *same* `arenaLiouville`, records are context-fixed and dynamically created (superseding the preparation-indexed `vnPointerOutcome` readout), Lüders is the *dynamical* pushforward (superseding conditioning-as-prediction), and the isolate→measure→isolate round trip is a theorem (`arena_round_trip`). The `FiniteQMClosure` witness above stands unchanged as the historical operational capstone. |
...
- **L1** — the Kähler-geometry fields now carry genuine formalizable content
  (`kahler_pointwise : IsFubiniStudyKahler`, the proved pointwise FS Kähler
  compatibility; `liouville_isProbability` = normalized volume; concrete fields
  since the 2026-08-06 F-04 tightening). The
  **manifold** residual (closedness `dω=0`, top-power volume identity) is proved and
  wired since 2026-09-10 (`LF4/SectorManifold.lean`, W1) — L1 DISCHARGED.
...
**(The "both pillars on one posited sector" claims are CONNECTED forward results, so
they are permitted; the forbidden line is: do not claim the sector/Σ is *derived*, and
do not sell the epistemic witness as completing the reconstruction. Σ is the floor.)**
...
- ✅ "A single posited Kähler sector object yields BOTH the Born rule AND
  ...
  (`manyToOneSchrodingerSetup_both_pillars`). Must carry the posited-sector caveat
  (Σ is the floor; the sector is posited, not derived).
- ✅ "One ontic model `(Σ, μL, Φ, π)` carries isolated dynamics, de-isolating
  ...
  *(Since 2026-08-02 the stronger one-arena form is also available: `unifiedArenaClosure`, see the L9 row addendum.)*
...
- ❌ "CSD DERIVES the sector `(π, G)` / the Born weights FROM the deterministic
  flow" / "the sector is no longer posited" — **FORBIDDEN**. Σ is the floor; the sector
  is posited, not derived (deriving it is a non-question). Every CONNECTED claim above is
...
- **C1 (fixes L4) — DONE 2026-07-07** (`LF4/NonTrivialSetup.lean`). Built
  ...
  measure-preserving via `fsMeasure_smul_invariant`) and the concrete
...
- **C4 (fixes L5/L6 structurally) — DONE 2026-07-07** (`LF4/BothPillars.lean`).
  ...
  the sector field `d.liouvilleMeasure` (defeq `fsMeasure`);
...
**Phase 3 — the record layer (kinematic interface BUILT 2026-07-25; MD-1 CLOSED 2026-08-31, residuals `R-015` boundary / `R-016` open) + the extensions.**
...
- **C6 (retired as stated).** "Derive the Born weights FROM the deterministic flow" is a
  **non-question** — Σ is the floor; Born *arises from* the Ω-region volume ratios +
  ...
  (`record-layer-plan.md §4`); its residual is context-fixing the *pinned closure's* regions (still the
  preparation-indexed `bornRegion ψ'`), and realizing **the record** as the ontic selection in Σ (which
