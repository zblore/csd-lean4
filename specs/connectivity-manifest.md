...
remains OPEN is the ONE deep link **L7 / A5 / SL-1**: the sector and its Born
...
- **L7** ★ — the DEEP gap: the Born trials sample the sector's measure i.i.d.;
  ...
  *derived from the dynamics*. This is the sector-origin problem (A5→D1, SL-1),
...
SL-1) — the Born trials still SAMPLE `kMuL`, and the fibre flow is trivial
...
- ❌ "CSD DERIVES the sector `(π, G)` / the Born weights FROM the deterministic
  ...
  A5 / SL-1 is OPEN; the trials SAMPLE `μL` i.i.d., the sector is posited). Every
...
**Phase 3 — the thesis frontier (deep; = SL-1 / A5→D1).**
- **C6 (fixes L7, makes L5/L6 genuine).** Derive the Born weights FROM the
  ...
  `future-work.md` SL-1). It is the real prize and is research-grade.
