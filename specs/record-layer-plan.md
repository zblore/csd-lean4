...
| 4 | The context-fixed regions `{Ωᵢ(M)}` def + μ_FS-null boundaries (Voronoi). *Priced 2026-09-13 as **RL-2** (`BACKLOG.md` ▶ OPEN QUEUE #6): audit S — no Voronoi / base-level context-fixed theorem exists in `RecordLayer/` or `SigmaLayer/`; the landed null seams (`NullSeamGeneralN.lean`) are on the register circle and the landed context-fixed partition is the fibred `globalBasin` — then the seam-nullity geometry M. Geometric half only; Born on these base regions is the parked base-only A7.* | low |
