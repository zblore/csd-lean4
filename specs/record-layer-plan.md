...
- **The record layer on the ACTUAL projective Σ (migration) — LANDED 2026-07-25:**
  ...
  `bornRegion` (events), `bornOutcome` (outcome map), and `fsMeasure`: `projRecordSemantics`
...
| ~~4~~ | ~~The context-fixed regions `{Ωᵢ(M)}` def + μ_FS-null boundaries (Voronoi).~~ **SUPERSEDED 2026-09-14.** This step was written to Paper C Appendix D's FS-Voronoi picture before Phase 1 (row 1 above) showed base-sharp regions cannot carry Born at `N = 3` and §3b dropped them for the fibre. The context-fixed partition the corpus actually carries is the fibred `globalBasin` (`RecordLayer/GlobalBasin.lean`; measurable, preparation-free, Born via `globalBasin_born`) and its null boundaries are the register-circle seams of `RecordLayer/NullSeamGeneralN.lean`. The 2026-09-13 pricing as RL-2 (`BACKLOG.md` ▶ OPEN QUEUE #6) is withdrawn; no Lean module uses Voronoi. | — |
