...
**PUBLISHED 2026-09-12:** https://github.com/zblore/csd-qit-bridge (public; commit `8d5b77a`; first CI run green in 7m29s: five `csd-lean4` modules and physlib's SSA cone compiled from source, then the bridge and its pins). The lakefile requires Mathlib, `csd-lean4` (`6b88f5f`) and physlib (`8b2b237`) by pinned git revision with the rule that the two pins move together; CI builds the bridge with warnings fatal and then its axiom-pin target, compiling `csd-lean4` and physlib's SSA cone from source (neither publishes oleans a dependent workspace can restore; Mathlib comes from its cache). The §3b tax is now live and is paid in that repository. **What the bridge does not unlock, corrected here:** the BCFJS commuting-states `iff` of `NoBroadcasting.lean` was priced as an M "once DPI is in hand"; that was wrong. Its only-if direction needs fidelity monotonicity (the BCFJS proof) or the *equality case* of data processing, Petz recoverability (the relative-entropy proof), and physlib carries neither. It is an L on either library and stays a priced row, not a rider of this bridge.
