...
**Status:** SCOPED 2026-09-20, after rungs (a) and (b) of row 36 landed (2026-09-19/20) and a survey of the
...
| multiplication by `L^∞` on `L²` | ✓ as a fact (`MemLp.mul`, Hölder), ✗ as a bundled `Lp 2 →L Lp 2` operator | probe P1 (§7): package the multiplier as a CLM; needed for `M_V` and for the Fourier multiplier |
...
| Riemann sums of a continuous function | ✗ as a ready lemma (`BoxIntegral` proves Riemann integrability, heavier than needed) | probe P3 (§7): uniform continuity on `[0, t]` gives the lemma in ~40 lines |
...
FC-6 lands with FC-4: the sentence in `docs/FROM-POSTULATES-TO-QUANTUM-COMPUTERS.md` that reads "The continuum
path integral is not in the corpus" is replaced by the Feynman–Kac citation, and the row 36 entry in
`specs/future-work.md` (FP-1) records the continuum rung as done in the Euclidean form and in Nelson's form.
...
3. **FC-2** (M) and **FC-3** (M), in either order.
