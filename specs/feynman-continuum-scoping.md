...
3. **FC-3** (M).
   **FC-4 outcome (2026-09-20): landed as `Mathlib/Probability/FeynmanKac.lean`, 6 pins, M–L took L.** Exactly the route of §4: the multiplier exponential `exp(−h M_V) = M_{e^{−hV}}` by the operator series applied to `f` (partial sums pointwise, `L²` limit and pointwise limit identified on finite-measure sets), the Wiener-side limit by dominated convergence with the Riemann sums of probe P3 along the continuous path, the operator side by FC-2's Trotter formula, and the two limits identified on every finite-measure set. Bounded `f`; the `L²` extension by continuity is FC-4′ (S). Conditional on `hB : IsBrownianReal B P`. **FC-4′ outcome (2026-09-20, BACKLOG #42): landed as `Mathlib/Probability/FeynmanKacL2.lean`, 7 pins, S took M.** "By continuity" was the right idea and the wrong price: the operator side is a bounded operator, but the continuity of the Wiener side `g ↦ E[e^{−∫V} g(x+B_t)]` on finite-measure sets needs `E‖g(x+B_t)‖ = (P_t|g|)(x)` a.e. (the pointwise formula of FC-2 plus the law of `B_t`) and then Cauchy–Schwarz against the indicator and the contraction `‖P_t‖ ≤ 1`; the functional is only defined for a.e. `x` (Fubini on FC-2's shift integrability), and its measurability comes from the sliced functionals, whose limit lemma was generalised from bounded `f` to `f` integrable along the endpoint. Density of the simple functions closes it. Snag: `rw` could not match `exp (h • −M_V)` across the two modules' instance paths for the ℝ-action on operators; `congr 1; exact` did.
4. **FC-4** (M–L): Feynman–Kac. The Euclidean deliverable.
   **FC-5 outcome (2026-09-20): landed as `Mathlib/Analysis/Semigroup/SchrodingerGroup.lean`, 12 pins, L took M–L.** The multiplier P1 was already the pin's `holderL`. The one delicate item, the strong continuity of the phase group `M_{e^{−itκ}}`, needed no `L²` dominated convergence: weak continuity `⟪M_t f, f⟫ → ⟪f, f⟫` is a scalar dominated convergence with dominator `|f|²`, and the isometry plus polarisation, `‖M_t f − f‖² = 2‖f‖² − 2 Re ⟪M_t f, f⟫`, give the strong limit. The Fourier side is conjugation by `Lp.fourierTransformₗᵢ`, nothing else. `of_group`, `perturbed` and `tendsto_trotterStep_pow_apply` then give Nelson with the kick identified by the complex multiplier exponential `exp_eq_potential` (FC-4's proof for a bounded complex `m`). Beyond the row's cell: the propagator is an isometry (a strong limit of isometries) and **unitary** — the reversed dynamics `(−κ, −V)` is a two-sided inverse, because the inverse of the forward Trotter approximant `(U M)ⁿ` is the conjugate `M⁻¹ (U⁻¹ M⁻¹)ⁿ M` of the reversed approximant and both converge strongly. Stated for any real measurable dispersion relation `κ` (`freeSchrodinger` is `κ = 2π²ξ²` in Mathlib's Fourier convention). The item of the cell not done is the explicit action on Gaussians/Schwartz functions — FC-5″ (S–M), through `SchwartzMap.toLp_fourierInv_eq`. **FC-5″ outcome (2026-09-20, BACKLOG #43): landed as `Mathlib/Analysis/Semigroup/SchrodingerSchwartz.lean`, 8 pins, S–M took M.** Mathlib's `SchwartzMap.fourierMultiplierCLM` does the Schwartz side once the phase `e^{−itκ}` is known to have temperate growth (a composition: `s ↦ e^{is}` has unimodular derivatives, `κ` is temperate); `U_κ(t)` then preserves Schwartz space and `H₀ = −½Δ` is Mathlib's `laplacian_eq_fourierMultiplierCLM`. Beyond the cell: the free Schrödinger equation `∂_t U₀(t)f = −i H₀ U₀(t)f` holds in `L²` at every `t` for Schwartz `f` — a dominated convergence for the difference quotient of the phase (`|(e^{−itκ}−1)/t| ≤ |κ|`, dominator `4κ²|𝓕f|²`), lifted through the Fourier isometry, moved by the group law. The Gaussian packet is not done: the pin has no Gaussian bundled as a `SchwartzMap` (BACKLOG #48, M). **FC-5‴ outcome (2026-09-20, BACKLOG #48): landed as `Mathlib/Analysis/Semigroup/GaussianPacket.lean`, 11 pins, M took M.** The Schwartz Gaussian was, as priced, most of it: the `n`-th derivative of `e^{−πax²}` is `P_n(x) e^{−πax²}` with `P_{n+1} = P_n' − 2πa X P_n` (differentiate on `ℂ`, then `HasDerivAt.comp_ofReal`), a polynomial is bounded by `∑ |c_i| |x|^i`, and `|x|^m e^{−cx²} ≤ 1 + m!/cᵐ` comes from Mathlib's `yᵐ/m! ≤ e^y`. The rest is Mathlib's `fourier_gaussian_pi` read through `SchwartzMap.fourier_coe` and `fourierInv_coe` (the inverse transform of an even function is its transform, `fourierInv_eq_fourier_neg`), the phase `e^{−2π²itξ²}` absorbed into the exponent, and one branch-cut fact: `(xy)^r = x^r y^r` when `Re x, Re y > 0` (`Complex.arg_mul` under `|arg| < π/2`), which gives the amplitude `(1 + 2πiat)^{−1/2}`. One dimension only (`E = ℝ`; `EuclideanSpace ℝ Unit` is not `ℝ`, and the `d`-dimensional packet would go through `fourier_gaussian_innerProductSpace`). Snag: `Lp.coeFn_smul` for the real action would not `rw` against the CLM-level `h • B` of `BoundedPerturbation.lean` (whose ℝ-action is `NormedSpace.complexToReal`); a `show` with the complex scalar `(h : ℂ) •` resolved it, the two actions being definitionally equal.
