...
Mermin). LF6-5 DONE (general-`d` CGLMP bound + tightness, d-intrinsic non-factorisation); LF6-7 DONE (Φ⁺↔ψ⁻ transport recompute); LF6-6 PARTIAL (partial-Schmidt correlation done; non-factorisation-for-unequal residual needs Gisin); LF6-2 PARTIAL (T2 dephasing semigroup done; general Lindblad generator + T1 + CP residual). All LF6 residuals now have their bounded cores landed.
