...
| EC-2 — **cost side DONE 2026-07-09** (value side deferred) | Safegcd reversible circuit: `divstepGadget` with `denote = divstep`, cost over a proven circuit | divstep + Reversible DSL | **M/L** | **Cost side done** (`SafegcdInversion.lean`): `divstepProxyGadget` EXHIBITS the divstep gadget as one concrete `Circuit` (`modSub ++ cuccaroCModAdd ++ cuccaroModDouble`), and `divstepProxyGadget_toffoli` proves its Toffoli cost IS `divstepToffoli n` — the cost is now the cost of a real in-DSL circuit, not a count over a hypothetical one. AxiomAudit-pinned. **Value side deferred (the large residue):** the divstep does PLAIN-integer subtract/halve on shrinking `f,g` (the modular gadgets are same-O(n) proxies whose `denote` ≠ integer divstep), AND `divstep` is not injective (`divstep 0 1 2 = divstep 0 1 1`), so a value-faithful `divstepGadget` (`denote=divstep`) needs signed-register arithmetic + garbage/history bits — a genuine multi-session build. |
