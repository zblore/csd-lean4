...
| ~~**W8**~~ **DONE 2026-09-11** (Cat-1 `Mathlib/QuantumInfo/Concavity.lean`: ★ `vonNeumannEntropy_mixture_ge` via `S(σ) − ∑ pᵢ S(ρᵢ) = ∑ pᵢ D(ρᵢ‖σ) ≥ 0` (Klein), `holevoChi` + ★ `holevoChi_nonneg`; applied in `LF2/PreparationCoarseGraining.lean`: ★★ `preparationEntropy_mixture_ge`, W4's coarse-graining half — 11 pins, took S. Klein's full-support condition **removed 2026-09-12**: `Mathlib/QuantumInfo/ConcavityFull.lean`, `vonNeumannEntropy_mixture_ge` with no support hypothesis, by mixing with the maximally mixed state (explicit spectrum) and closedness in `ε`; `preparationEntropy_mixture_ge` and `holevoChi2_nonneg` now hypothesis-free) | **Concavity of von Neumann entropy** (`S(∑ pᵢ ρᵢ) ≥ ∑ pᵢ S(ρᵢ)`): needed for Holevo ≥ 0, for W4's coarse-graining, and for any capacity statement. The corpus has `matrix_log_concave`/`matrix_rpow_concave` (`OperatorConvexBridge.lean`) but not `S`; Mathlib has `CFC.concaveOn_log` since 2026-08-30. Route: `S(ρ) = −Tr(ρ log ρ)` and operator concavity of `x log x`'s negative via the CFC. | **M** | Medium–high | Medium |
| ~~**W9**~~ **DONE 2026-09-12** (route (b): Gate 0 passed — physlib's SSA, joint convexity and DPI are foundational-triple on our exact pins; the bridge package `csd-qit-bridge` (`C:\zayn\csd\qit-bridge`, outside this corpus, path dependencies, 8 pins) exports `CsdQitBridge.strong_subadditivity` — SSA for every density matrix in the corpus's vocabulary, no support hypothesis — and `CsdQitBridge.relEntropy_traceC_le`, the corpus's `hDPI` verbatim, so `strong_subadditivity_of_relEntropy_monotone` is applied there (`strong_subadditivity_of_posDef`). Took M. **This corpus is unchanged**: `hDPI` stays explicit, CL-023 stays qualified-by-design, the Mathlib-only footprint holds. **Published 2026-09-12** at https://github.com/zblore/csd-qit-bridge (by-revision requires, own CI). The BCFJS `iff` is not a rider of the bridge: it needs fidelity monotonicity or the equality case of DPI, absent from both libraries (L, its own row). `lieb-dpi-scoping.md` §6.) | **SSA unconditional** — the `hDPI` hypothesis of `strong_subadditivity_of_relEntropy_monotone`. `lieb-dpi-scoping.md` (2026-09-01) found physlib's `Sᵥₙ_strong_subadditivity` sorry-free on identical pins and recommends *leave `hDPI` honest, or bridge*; its Gate 0 (`#print axioms`) is unrun. Do that gate before pricing further. | **L–XL** (build) / **S** (bridge if Gate 0 passes) | Low / High | Medium |
| ~~**W10**~~ **DONE 2026-09-11** (13 pins; Cat-1 `Mathlib/QuantumInfo/HolevoBound.lean`: the Holevo bound `holevoChi_le_log_card` with no support hypothesis, `holevoRange`; `LF6/DeisolationCapacity.lean`: ★★ `deisolationChannel_holevoCapacity : IsGreatest (holevoRange (deisolationChannel 2)) (log 2)` — the single-letter Holevo capacity of the de-isolation channel is one classical bit, a theorem on the genuine channel; `ChannelCapacity.lean`'s `holevoChi2_nonneg` now asserted via W8. Took S. Regularised capacity not defined.) | **Holevo χ and one capacity.** With W8: `holevo_nonneg`, `holevo_le_log_dim`, and the single-letter classical capacity of the dephasing witness as a *theorem* rather than an example (`ChannelCapacity.lean` has the arithmetic). Regularised capacity not in scope. | **M** after W8 | Medium | Low–medium |
| ~~**W11**~~ **DONE 2026-09-11** (12 pins; `QM/QEC/BitFlipDilation.lean`: the bit-flip channel IS `stinespringChannel U_p |0⟩` as channels; `CSD/QEC/ThreeQubit.lean` rewritten: ★★ `bitFlipFlow_traceRight_barycenter` — the bit-flip channel is the environment marginal of the concrete `Σ`-flow `bitFlipFlow` on `cpSectorData` over `ℂℙ³`, no lift hypothesis; ★★ `errorRegion_disjoint` — the code space is a region of `Σ` whose four error images are pairwise disjoint (the syndrome is a partition of `Σ`); `csd_three_qubit_corrects_single_bitflip` and `csd_qec_decoherence_corrected` carry no bundle binder; `CSDThreeQubitBundle` deleted. Took M. **Residue closed 2026-09-12:** syndrome-conditioned recovery as one channel on the mixed state — `QM/QEC/SyndromeRecovery.lean` (13 pins), ★★ `recoveryChannel_apply_singleFlipChannel_apply`, consumed by `syndrome_recovery_corrects_mixed`, the last conjunct of `csd_qec_decoherence_corrected`; `syndromeProj_fixes_errorRegion` identifies the syndrome projectors with the error regions. Took M. **Register ⊗ environment closed 2026-09-12:** `QM/QEC/RegisterDilation.lean` (the single-error channel is the Stinespring channel of `registerUnitary = blockDiagonal errorOp · (1 ⊗ R_q)` with a four-level ready environment) + `CSD/QEC/RegisterFlow.lean` (`registerFlow` on `ℂℙ³¹`, ★★ `registerFlow_traceRight_barycenter`, ★★★ `registerFlow_recovery` — QEC on `Σ` end to end; 23 pins). Took M. **Independent noise closed 2026-09-21 (#52):** `QM/QEC/ControlledDilation.lean` (the generic controlled-error dilation), `QM/QEC/IndependentNoise.lean` (★★ `recoveryChannel_apply_indepFlipChannel_apply`: `R (N_p ρ) = (1 − p_fail) ρ + p_fail X̄ ρ X̄`, `p_fail = 3p² − 2p³`) + `CSD/QEC/IndependentNoiseFlow.lean` (`indepFlow` on `ℂℙ⁶³`, ★★★ `indepFlow_recovery`); 34 pins. Took M.) | **QEC on Σ.** `three_qubit_corrects_single_bitflip` with the bit-flip channel produced by W6 from a Σ-flow, and the code space read as a Σ-region (`RecordLayer` basins). Removes the unused `_bundle` binders honestly. | **M** after W6 | Medium | Medium |
| ~~**W12**~~ **DONE 2026-09-11** (both mis-tags re-tagged 3-Local — "6-Local" is not a CONVENTIONS §1 category; **the whole corpus swept and guarded 2026-09-12**, `scripts/check-category-tags.sh`, BACKLOG hardening row; the three stale notes corrected; `EMPIRICAL.md`'s QEC row now says its bundle is unused) | **Ledger fixes now, no Lean:** re-tag `ChannelCapacity.lean` and `Einselection.lean` from "6-Local" to what they are; mark the unused-binder twins as such in `EMPIRICAL.md`; the three stale notes the survey found (`StrongSubadditivity.lean:80` — the fork is no longer build-vs-axiom; `Pauli.lean:28` and `Clifford.lean:37` say stabiliser measurement is "not attempted" while `Stabilizer.lean` proves it; `channels-plan.md:104` lists DPI as deferred, it landed 2026-06-09). | **S** | High | Site accuracy |

**The critical path is W2 → W3 → W6 → W7; all four built 2026-09-11; every row W2–W12 is now done (W9 outside the corpus, 2026-09-12).** From CSD's posited sector and a flow lifting a unitary, the density operator of a preparation, its evolution, the channels it passes through, and the second law, data processing and Landauer bounds on it are now one chain of theorems. After W2 (an afternoon) every entropy, distance, and DPI theorem
...
theorems rather than a chain of headers. W8–W11 extend the reach; W9 was a Mathlib-depth fork the author
ruled on by picking it: the bridge, not the build.
