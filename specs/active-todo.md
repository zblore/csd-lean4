...
| 36b | Carry-clean VerifiedAdder variant (global restored-clean invariant) + instantiate the existing cuccaroModMul step → recover `20n²+14n` by instantiation (FULLY Boolean-verified, no amplitude wall); first instance that moves the cost | ECDSA | M | open | #36a |
| 36c | Generic inverter (safegcd parametric) + instantiate → the dominant 67% inversion term moves | ECDSA | L | open | #36a,#36b |
