...
| [`nqubit-register-plan.md`](nqubit-register-plan.md) | Plan for the **n-qubit register** + quantum-algorithm branch (Deutsch–Jozsa → QFT → Grover → Shor). `QReg n = EuclideanSpace ℂ (Fin n → Fin 2)`; Cat-1 in `Mathlib/QuantumInfo/Register.lean`, algorithms in `Empirical/QM/Algorithms/`. R1 register → R2 `Hn\|0ⁿ⟩` → R3 character-orthogonality/`Hn` unitary → R4 Deutsch–Jozsa. | LIVE. **R1 + R2 DONE 2026-06-05** (`Register.lean`: `QReg`/`basisState`/`prob`, Born=inner-sq, normalisation; `Hadamard.lean`: `Hn`=H^⊗n, `Hn_apply_zero`=uniform superposition; AxiomAudit-pinned). Next: R3 char-orthogonality ⟹ `Hn` unitary → R4 Deutsch–Jozsa. Shor's tail is a large *classical* number-theory build. Coverage breadth, not the thesis. |
