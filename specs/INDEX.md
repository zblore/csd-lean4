...
| [`k1-plan.md`](k1-plan.md) | Staged plan for **K1 — von Neumann entropy `S(ρ) = −Tr ρ log ρ`** and the entropy/information stratum (subadditivity, SSA, data-processing, downstream Holevo/Schumacher/thermo). Mostly Cat-1 `QuantumInfo` infrastructure *beside* the CSD layers; the only thesis-relevant slice is the Landauer / ontic-entropy touchpoint (`S = log de-isolated volume`, `↔ μ_L`). K1-A reuses `TraceDistance.lean`'s `IsHermitian.cfc` + `re_trace_cfc` + eigenvalues. | **K1-A DONE 2026-06-16** (`Mathlib/QuantumInfo/Entropy.lean`): `vonNeumannEntropy` + operator-form `−Tr(ρ log ρ)` + `S ≥ 0` + pure-state `S = 0` + unitary invariance, foundational-triple-only, AxiomAudit-pinned. **Tensor additivity** stated under an explicit eigenvalue-product hypothesis (no Kronecker spectral theorem in Mathlib; full discharge = deferred K1-A.2); `S ≤ log d` deferred (Jensen). **K1-C strong subadditivity is the hard new-math core** (Lieb concavity, likely not in Mathlib) with an OPEN FORK (build operator-convexity vs axiom-state SSA), deferred. |
