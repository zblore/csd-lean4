...
`no_everywhere_correlation` (`RecordLayer/MeasurementConstraints.lean`) makes
...
|---|---|
| FS measure invariance under the rotation | `fsMeasure_smul_invariant` (one line, as in `joinSwap_measurePreserving`) |
...
[`future-work.md`](future-work.md); `RecordLayer/PiecewiseHamiltonian.lean` (the flux
correction), `RecordLayer/ShearDiscontinuity.lean` (`shearEvolve_not_continuous`),
`RecordLayer/MeasurementConstraints.lean` (`no_everywhere_correlation`),
`RecordLayer/GlobalBasin.lean` (`ContextField`), `RecordLayer/TorusFibre.lean`
(`volume_torusCell`).
