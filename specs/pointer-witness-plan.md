...
| 4b | `ε`-Born sector sandwich (`rⱼ − 2ε ≤ μ(sector j) ≤ rⱼ + 2(N−1)ε` for the ready-conditioned epistemic preparation, via `volume_shrunkCell_slice` + disjointness — no upper-bound cell geometry) and the smooth-horn closure statement | M |
