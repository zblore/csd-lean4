...
| 2b | Weight field: `wⱼ(p,θ)` trapezoid bumps on `ε`-shrunk context cells (continuous in `(p,θ)`; the moving-endpoint circle geometry), the arena propagator, its **joint continuity** (the theorem the piecewise witness provably lacks — contrast `shearEvolve_not_continuous`), slice-wise measure preservation ⇒ product-measure preservation | M |
| 3 | Landing theorem on shrunk cells; two-time law; persistence; `MeasurementProtocol` instance | M |
| 4 | `ε`-Born sector accounting; the smooth-horn closure statement | M–L |
