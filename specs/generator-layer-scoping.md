...
| **G18** ✅ built 2026-09-21 (Q31, in Moser's form) | **Darboux's theorem** (the `TERMS.md` symplectic line): every symplectic form is locally the standard one. Moser's trick: needs G5(a) flows, G5(b) Cartan, and a Poincaré lemma on a ball. **Built as the constant form `ω(x₀)` in a `C¹` chart with `C¹` inverse** (`Mathlib/Geometry/Manifold/Darboux.lean`; the `C¹` chart `BACKLOG.md` #49, 2026-09-23) **and in the standard form `∑ dpᵢ∧dqᵢ` on `ℝ^{2n}`** (`DarbouxStandardForm.lean` with the symplectic basis of `LinearAlgebra/BilinearForm/SymplecticBasis.lean`, `BACKLOG.md` #50, 2026-09-23); the `C^k` chart of a `C^k` form is `BACKLOG.md` #60. | ~~**XL**~~ **M** took M | Low | Low | Nothing in the corpus consumes it; it is the classical theorem a symplectic library owes. |
...
the `C¹` chart and the standard form, landed 2026-09-23 as #49 and #50; the `C^k` chart is #60.)*
...
`LinearAlgebra/BilinearForm/SymplecticBasis.lean`, composed with the Moser chart). Residue: a `C^k`
form gets only a `C¹` chart (`C^k` dependence of flows, #60). The paragraph below is the pricing as it
stood.)*
