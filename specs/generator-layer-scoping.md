...
| **G18** ✅ built 2026-09-21 (Q31, in Moser's form) | **Darboux's theorem** (the `TERMS.md` symplectic line): every symplectic form is locally the standard one. Moser's trick: needs G5(a) flows, G5(b) Cartan, and a Poincaré lemma on a ball. **Built as the constant form `ω(x₀)` in a `C^n` chart with `C^n` inverse for a `C^n` form, `C^∞` on a symplectic manifold** (`Mathlib/Geometry/Manifold/Darboux.lean`; the `C¹` chart `BACKLOG.md` #49 and the `C^n` chart #60, 2026-09-23, through `Analysis/ODE/FlowSmooth.lean`) **and in the standard form `∑ dpᵢ∧dqᵢ` on `ℝ^{2n}`** (`DarbouxStandardForm.lean` with the symplectic basis of `LinearAlgebra/BilinearForm/SymplecticBasis.lean`, `BACKLOG.md` #50, 2026-09-23). | ~~**XL**~~ **M** took M | Low | Low | Nothing in the corpus consumes it; it is the classical theorem a symplectic library owes. |
...
the `C¹` chart and the standard form, landed 2026-09-23 as #49 and #50, and the `C^n` chart as #60.)*
...
variational solution on the initial point, `dist_le_of_linearODE_coeff_close`), and the standard form
`∑ dpᵢ∧dqᵢ` is `DarbouxStandardForm.lean` (#50, landed 2026-09-23: a symplectic basis of `ω(x₀)`,
`LinearAlgebra/BilinearForm/SymplecticBasis.lean`, composed with the Moser chart). The `C^n` chart of a
`C^n` form is #60 (landed 2026-09-23: `C^n` dependence of flows on the initial point,
`Analysis/ODE/FlowSmooth.lean`, and the `C^n` Poincaré primitive through
`Analysis/Calculus/ContDiffParametricIntervalIntegral.lean`). The paragraph below is the pricing as it
stood.)*
