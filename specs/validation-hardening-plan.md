...
| K | Standard linter integration | Baseline-first per the §9.5 ratchet; F3 naming decisions on the documented-exclusions list from day one; advisory unless zero-churn | **DONE 2026-08-12** — `scripts/check-std-lint.sh` (Batteries `runLinter` over the corpus root, **25 s measured** on a built tree) in CI's advisory step. **Advisory by explicit decision, recorded in-script**: (1) F3 settled defect classes as fix-on-touch, never swept — a blocking linter would force that sweep; (2) documented literature-notation exceptions exist. First run reports a modest `unusedArguments` list (unused instance binders) — the on-touch class, now visible every CI run. **RATCHETED 2026-09-04** (hardening session, CONVENTIONS §9.5): per-class counts pinned in `docs/std-lint-baseline.txt`, the guard fails on GROWTH only, so the no-sweep decision above is untouched. The pass that preceded the pin took `simpNF` 18 → **0** (18 dead simp lemmas; root causes `Channel.apply_def` and `Kahler.complexStructure_apply`, head unfolders that pre-empted seven content lemmas between them) and `docBlame` 20 → **0**; the five unused-*hypothesis* findings — the class the necessity audit found by hand — were each read and are documented uniform-interface bundles, so no stated assumption in the corpus is unexplained. Mutation-probed in `check-guards.sh` (probe 11) |
| L | Import hygiene guard | Extend the `check-import-negative.sh` idiom; permanent mutation cases in `check-guards.sh` | **DONE 2026-08-12** — `scripts/check-import-hygiene.sh` (**blocking**, in ci.yml): (1) no bare `import Mathlib` in production; (2) production never imports `Tests` (witness ≠ result, structurally); (3) stable→`Incubator` imports must match the declared seam inventory (the four §H seams — a new leak is a visible diff, not a silent architecture change). Three permanent probes (10a–10c) + clean-tree check added to `check-guards.sh` — 11 probes green |
| M | Forward-compat workflow | **Tagged Mathlib releases only** (cache hits); nothing committed to lakefile.toml | **DONE 2026-08-12** — `.github/workflows/compat.yml`: weekly + manual, never on push/PR (pinned baseline untouched); takes the toolchain **from the tested Mathlib ref itself** (coherent pair), repoints rev in the workspace only; builds both targets + core static controls. **First live run same day, two tunings applied:** (a) `--wfail` REMOVED from the forward build — against master, deprecation warnings fire always and would saturate the canary (the run proved it: 3.7k+ jobs built, failure was purely `if_true`→`ite_true` deprecations — which is itself the useful finding: the next Mathlib bump owes that sweep); (b) macOS job moved onto `lean-action` (hand-rolled elan died, exit 127). **Canary log, 2026-08-17/24/29:** 08-17 red = real forward break (`Measurable.abs` arriving only transitively; fixed 08-20 with direct imports, dispatch green). 08-24 red = cache flake (partial master cache; workflow hardened 08-29 with a double `cache get` pass). 08-29 dispatch red = two real master drifts, both fixed same day with both-tree spellings: master removed `Measure.isProbabilityMeasure_map` (now an unconditional instance, no alias) — corpus shim `Mathlib/MeasureTheory/MapProbability.lean` (`isProbabilityMeasure_map'`, proved from stable `map_apply_of_aemeasurable`), five call sites rerouted; and `cond_map`'s rfl-closure drifted under master's `map_smul` normal form — closer made shape-tolerant (`simp`). **When the pin advances past these master changes, delete the shim and collapse its call sites to `inferInstance`.** **Sweep discipline, learned the hard way same day:** when the canary names a broken lemma, fix by CORPUS-WIDE REGEX SWEEP of that name (word-boundary, newline-tolerant), never by the reported-site list — the first fix missed twelve further sites in three rounds (early build death masks later modules; space-delimited matching misses line-broken names; a `head`-truncated verification grep hid five more). The verification grep must be untruncated and must return empty. Second drift, same canary: `Measure.map_smul` gained a measurability hypothesis on master (the unconditional form is false under master's new `map` junk value) — `Measure.map_smul'` shim (Measurable-hypothesis form), six sites swept. **Canary log, 2026-09-14 (scheduled run 34835270648, Mathlib master `cf6bde1` on Lean `v4.34.0-rc2`):** red on four modules, and a local master worktree (`lake update` + double `cache get`, then `lake build CsdLean4`) found the two the early death had masked. All six fixed with both-tree spellings, verified on the pin and on master: (i) `Matrix.PosSemidef.dotProduct_mulVec_zero_iff` moved to `LinearAlgebra/Matrix/PosDef.lean` with the vector IMPLICIT — seven sites now pass it as the named argument `(x := v)`, which elaborates against an explicit or an implicit binder; (ii) `measurable_pi_lambda` became a deprecated alias of `Measurable.of_eval` with the function implicit — eleven sites swept to `measurable_pi_iff.mpr`; (iii) `MDifferentiableAt.mfderiv` now wraps the chart `fderivWithin` in the definitional `tangentSpaceCastModel` identifications — `ExteriorDerivative.lean` strips them with a `change` to the pin's shape (a syntactic no-op on the pin); (iv) three further bare `Measure.map_smul` sites (the three `fsVolumeNormalized` invariance theorems, written after the 08-29 sweep) rerouted through the `map_smul'` shim, with `IsSymplectic.continuous_hamiltonianFlow` added to `HamiltonianFlowVolume.lean` for the measurability argument; the untruncated verification grep for bare `Measure.map_smul` is empty; (v) `mul_le_one₀` deprecated with no replacement — twelve sites inlined to `(mul_le_of_le_one_left hb₀ ha).trans hb`, its own proof; (vi) four transitively redundant imports dropped (`Data.Complex.Basic`, `Data.Real.Basic`, `Measure.MeasureSpace`), which master deprecates as modules. **What stays for the pin move** (BACKLOG HY-6): master's remaining warnings have replacement names that do not exist on the pin — Lean core `v4.34` renames `if_pos`/`if_neg`/`dif_pos`/`dif_neg`/`if_true`/`if_false` to `ite_eq_left`/`ite_eq_right`/`dite_eq_left`/`dite_eq_right`/`ite_true`/`ite_false` (~1,300 sites), `Mathlib.Data.{Complex,Real,ENNReal}.*` moved under `Mathlib.Basic.*` (8 imports), `stdSimplex` (a set) became the structure `StdSimplex` (15 sites, a real rework), plus `ContinuousLinearMap.lipschitz`→`lipschitzWith` (3), `Filter.eventuallyEq_set`→`eventuallyEqSet_iff` (2), `IsQuotientMap.secondCountableTopology`→`IsOpenQuotientMap.secondCountableTopology` (1). The `all_goals try rfl` at `FlowDerivative.lean:397` is flagged unused on master (its `convert` closes the goal there) and is needed on the pin. |
| N | Docs smoke workflow | Manual/scheduled; representative-page checks; failure visible | **DONE 2026-08-12** — `.github/workflows/docs.yml`: monthly + manual; runtime `docbuild` scratch project (doc-gen4 pinned to the toolchain tag, never committed); builds `CsdLean4:docs` and smoke-checks five representative pages (LF1 main theorem, LF2 transport, LF3 chain, LF6 obstruction, SigmaLayer Lüders) for existence **and** the declaration name. First scheduled run validates the runner plumbing |
| O | macOS platform build (optional, last) | Does not contradict G7 (Windows dropped because local dev covers it; macOS is uncovered) | **DONE 2026-08-12** — `macos-pinned` job inside `compat.yml` (scheduled/manual, non-blocking): both targets `--wfail` on the pinned toolchain |
...
| LF3 (singlet) | `perpContext` + `ofKählerPreparation` at it (`Witnesses/SingletBell.lean`) — concrete sector, preparation, context; `hgen` discharged (`P_st = ¼` computed) | `P_st_setting_dependent` (parallel axes give 0 ≠ ¼); all four weights ∉ {0, 1} | `LF3_singlet_frequency_convergence_joint` via `singletContextualPreparation`: exactly one recorded outcome per state; honest trials → all four frequencies ¼ simultaneously a.s. (review update 2026-09-22) |
| LF6/Bell | `kMuPsi_no_global_chsh_assignment` + `kMuPsi_chsh_obstruction_nonvacuous` on `(KSigma 4, kMuPsi)`; **upgraded 2026-08-13 (Q19)**: `kMuPsi_singlet_contextual_model` — the **positive half**, an explicit measurable family reproducing the singlet (full `P_st` table at every context) and incompatible with every global CHSH assignment | obstruction populated on BOTH conjuncts: compatibility (compatible families exist, correlation 1) AND — since Q19 — reproduction (the contextual model inhabits `ReproducesSingletAtCHSH`, which until then had no witness); the separation is two-sided | `no_compatible_global_chsh_assignment_realises_singlet` (→ `lhvCHSH_abs_le_two`), `compatibleGlobalCHSH_nonvacuous`, `c1_singlet_contextual_capstone` |
...
| `check-import-hygiene.sh` (WS-L) | probes 10a (bare `import Mathlib`), 10b (production imports Tests), 10c (undeclared Incubator seam) | **permanent** in `check-guards.sh` — all three fire, guard passes clean; runs every CI push |
| docs smoke (WS-N) | the smoke step fails structurally on a missing page or missing declaration name (`test -f` + `grep`) | validated by construction; first live run exercises it end-to-end |
