...
| R-005 | ~~#85~~ | pattern normalisation + the ancilla-free control-count recursion through square roots — **built 2026-09-26**, `QuantumInfo/ControlRecursion.lean` (★★ `ctrlSetOf_mem_closure`; the pair-gate algebra `pairSet_mul`, the block identity `diagBlock_five`, and `Euler.exists_sqrt`). **#72 closed in full: #83 + #84 + #85** | L | #83, #84 |
