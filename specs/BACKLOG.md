...
| **★ The record layer (MD-1)** — measurement as record *selection* on Σ | The near frontier of completing the Σ+Ω reconstruction of QM. Measurement = de-isolation = **record selection via context-fixed Ω-basins on Σ**: a partition of the prepared region `Ω₀` determined by the apparatus, preparation entering only through `Ω₀`, the record = *which basin the single trajectory occupies* — a selection of structure already in Σ (Σ does not grow). The corpus's `vnPointerOutcome` / `bornRegion ψ'` is epistemic + preparation-indexed on `ℂℙ` — the wrong side. **Endpoint:** separate preparation from context-fixed partitions and integrate the former over the latter, making measurement genuinely arise from Σ. | `CSD-CHARTER.md`, `reconstruction-status.md §7` |
