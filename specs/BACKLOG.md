...
| **★ The record layer (MD-1)** — measurement: context-fixed epistemic regions + the ontic record | The near frontier of completing the Σ+Ω reconstruction of QM. Two levels (Paper C A7): the apparatus context M fixes an **epistemic** outcome partition **{Ωᵢ(M)} ⊂ ℂℙⁿ⁻¹** (the Ω-regions are *epistemic*, not ontic), and **the record is the ontic selection in Σ** of which region the single trajectory realizes (which `π⁻¹(Ωᵢ(M))` basin `ω(t)` occupies; structure already in Σ). Born = the prepared density integrated over the fixed epistemic regions. **The corpus's gap (not "move regions to Σ"):** `bornRegion ψ'` is **preparation-indexed** (from the state), not **context-fixed** {Ωᵢ(M)} (from the apparatus M) — both epistemic; and the record is not yet the ontic selection. **Endpoint:** separate preparation from the context-fixed partition and integrate the former over the latter. **Qubit crux SOLVED analytically** (`record-layer-plan.md` §2: context-fixed hemispheres + spread `ρ_m = 4(m·λ)₊` → Born). **N≥3 Phase-1 DONE 2026-07-25 → Outcome B** (`record-layer-plan.md` §3): base-only reproduction fails at N=3 for FS-Voronoi regions — a covariant base density is forced NEGATIVE (numerical, `scripts/experiments/record_layer_base_only_test.py`; qubit control validates). **So the many-to-one fibre IS needed** (earned, not the retracted Gleason claim). **Next = the research core: the fibre construction** (`B_i(M)⊂Σ` using the fibre → Born, contextual). | `record-layer-plan.md`, `CSD-CHARTER.md` |
