...
| Item | Note | Source |
|---|---|---|
| ~~**SO-1 — "derive Σ / the sector origin"**~~ | **RETIRED — a non-question (2026-07-24).** Σ is the floor and is everything; there is nothing beneath it to derive it from. (Paper C is explicitly "a reconstruction, not a derivation"; μ_FS is already fixed by SU(n) symmetry, Paper B.) The frontier is not "where does Σ come from" but the record → spacetime layers below. Kept here only so it is not re-proposed as a goal. | `CSD-CHARTER.md`, `reconstruction-status.md §7` |
| **★ The record layer (MD-1)** — measurement as record *selection* on Σ | Measurement = de-isolation = **record selection via context-fixed Ω-basins on Σ**: a partition of the prepared region `Ω₀` determined by the apparatus, preparation entering only through `Ω₀`, the record = *which basin the single trajectory occupies* — a selection of structure already in Σ (Σ does not grow). The corpus's `vnPointerOutcome` / `bornRegion ψ'` is epistemic + preparation-indexed on `ℂℙ` — the wrong side. **Endpoint:** separate preparation from context-fixed partitions and integrate the former over the latter. These records feed the projection layer. | `CSD-CHARTER.md`, `reconstruction-status.md §7` |
| **★★ The projection layer — records → spacetime (THE thesis)** | The selected records in Σ **project to make spacetime** (emergent, non-fundamental). Σ is **local**; the record→spacetime projection is **non-local** ⟹ entanglement-at-a-distance is locality in Σ seen through a non-local projection — no fundamental action at a distance. This is the deliverable the whole programme is for. **Entirely unbuilt in Lean** — not even scaffolded. | `CSD-CHARTER.md`, Paper D §5.2 |
