...
| # | Paradox | Lean target | Status | Experimental verification |
|---|---|---|---|---|
| D6 | GHZ paradox (Mermin form) | 3-qubit `(|000⟩ + |111⟩)/√2`: classical local-realism cannot reproduce `⟨σ_x σ_x σ_x⟩ = +1` and the three permutations `⟨σ_x σ_y σ_y⟩ = ⟨σ_y σ_x σ_y⟩ = ⟨σ_y σ_y σ_x⟩ = −1` simultaneously | **DONE 2026-05-20** (`Empirical/QM/Multipartite/GHZ.lean`: `ghz_expectation_xxx`, `_xyy`, `_yxy`, `_yyx`, `no_lhv_assignment_for_ghz`) | Pan et al. 2000, *Nature* **403**, 515 |
...
| D9 | Kochen-Specker theorem | No non-contextual hidden-variable assignment consistent with QM on projection observables in dim ≥ 3 | **DONE 2026-05-20** (full, `Empirical/CSD/Contextuality/KS18.lean`: `no_value_assignment_18_9` + `ks_no_value_assignment_cabello18` for the combinatorial impossibility, plus `cabello_pairwise_orthogonal_in_basis` for the geometric content on the 18 ℝ⁴ vectors); QM-generic, Cat-2 candidate | Cabello 1996 (theoretical); experimental: Kirchmair 2009, others |
...
|---|---|---|---|---|
| ER2 | Ozawa error–disturbance | Universally valid uncertainty (Ozawa 2003): `ε(A)η(B) + ε(A)σ(B) + σ(A)η(B) ≥ ½\|⟪ψ,[A,B]ψ⟫\|`, replacing the Heisenberg-microscope form that Erhart 2012 / Rozema 2012 measured failing | **DONE 2026-09-04** (`Empirical/QM/Ozawa.lean` `ozawa_error_disturbance`, 6 pins; CSD side `Empirical/CSD/Ozawa.lean` `no_ozawa_model_of_jointLift` is a SCOPE theorem — the record layer carries no such model) | Erhart et al. 2012 (neutron spin), Rozema et al. 2012 (photon polarisation) |
| ER1b | Quantitative WAY (Ozawa 2002) | `ε(A)·σ(L) ≥ ½\|⟪Ψ,[A_in,L]Ψ⟫\|` — the measurement error bounded below by the conserved charge's spread; the structural WAY theorem is its `ε = 0` corner | **DONE 2026-09-04** (`Empirical/QM/QuantitativeWAY.lean` `quantitative_way`, 2 pins; WAY brick 2, rated L in the scoping note and actually an S once row B's definitions existed) | Ozawa, PRL 88, 050402 (2002) |
| ER1 | Wigner–Araki–Yanase | Finite-dim Araki–Yanase theorem (QM side, `Empirical/QM/`) + the CSD scope theorem (`Empirical/CSD/`, at the `IsJointLift` level): the stroke is a skew product on the arena, not a tensor-product isometry with an additive conserved `L`, so WAY's hypotheses have no record-layer instance — the LF5 `vnUnitary` tier meets them with an engineered `L` and WAY's conclusion holds there trivially (the fibrewise witness conserves *every* base function, non-commuting ones included, while recording exactly — outside WAY's hypotheses, not "compatible by construction") | **DONE 2026-09-02** (`Empirical/QM/WignerArakiYanase.lean`: `arakiYanase_identity`, `wigner_araki_yanase`, `no_exact_record_of_not_commute`; non-vacuity `way_hypotheses_satisfiable` on CNOT, sharpness `swap_exact_record_not_commute` on SWAP, instance `sigmaX_no_exact_conserving_record`; 7 pins). **Brick 1 DONE 2026-09-02** (`Empirical/CSD/WignerArakiYanase.lean`: `record_not_factor_hilbert` / `no_joint_hilbert_map` — at the `IsJointLift` level the pointer image is not a function of the Hilbert data, the fibre register selects the outcome via `IsJointLift.landing` (a robustness quantifier: `pointer_eq` pins every lift's pointer image to the witness's); non-vacuity `scope_hypotheses_satisfiable`, instances `pointerEvolve_record_not_factor_hilbert`, `jointLift_record_not_factor_hilbert`; 7 pins). Brick 2 (quantitative WAY, `specs/way-theorem-scoping.md` §3) behind the Ozawa row | Theoretical (Wigner 1952, Araki–Yanase 1960; Ozawa 2002 quantitative) |
...
| ER3 | Aharonov–Bohm / Berry phase | Phase holonomy on the fibre; the flagship phase experiment with no twin | **Scoped 2026-09-21** (`berry-phase-scoping.md`) **and the stateable half built**: the Aharonov–Anandan geometric phase with gauge invariance and the horizontal-lift holonomy (`Mathlib/Analysis/InnerProductSpace/GeometricPhase.lean`), Berry's spin-½ cone `−Ω/2` (`Empirical/QM/BerryPhase.lean`), and the curvature formula `β = −∫∫ 2 Im⟪∂_sΨ, ∂_tΨ⟫` with the cone's disc integral `Ω/2` (`GeometricPhaseCurvature.lean`, `BerryPhaseCurvature.lean`, BACKLOG #54, 2026-09-23); the fibre carries no holonomy of its own (the phase is a base quantity of the sector's closed orbits); the flux twin is the discrete AB ring, BACKLOG #55 | Chambers 1960, Tonomura 1986 (AB); Tomita–Chiao 1986 (Berry) |
...
| E3 (CPTP) | No-communication (reduced-density form, **arbitrary local channel**) | Alice's local **CPTP map** `Φ⊗id` (any channel, not just a unitary) leaves Bob's reduced state `traceLeft ρ` invariant | **DONE 2026-06-05** (channels phase C4; `Empirical/QM/NoCommunication.lean`: `channel_no_communication`). Built on the Kraus-summed partial-trace lemma `traceLeft_sum_conjTranspose_kronecker_one` + the local channel `QuantumInfo.Channel.tensorRight` (`Φ ⊗ id`). **Retires the E3 CPTP gap.** Foundational-triple-only, AxiomAudit-pinned. | Standard |
