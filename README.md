...
| **V** (volume series) | Born numbers *derived* (not transported) as Fubini-Study volumes: Stern-Gerlach, Malus, Bell, GHZ, Hardy, the trine / USD / SIC / qutrit / Hesse / MUB POVMs, and **any projective measurement context**, rank-1 *and* degenerate (`context_born_frequency_volume` / `block_born_frequency_volume` — the general contextuality grounding, Kochen-Specker + Mermin-Peres rank-2 eigenspaces as block sums of FS volumes), with the **Cabello-18 Kochen-Specker** context as a concrete instance (`ks18_context_born_frequency_volume`). Genericity-free since the 2026-06-11 hpos migration (`BornRegionUncond.lean`): boundary preparations — context eigenstates, the Mermin GHZ points `Φ = 0, π`, aligned Bell analysers — covered | `Empirical/CSD/{SternGerlach,Malus,Bell,GHZ,Hardy,Trine,USD,SIC,…}Volume.lean`, `Empirical/CSD/ContextVolume.lean`, `Empirical/CSD/Contextuality/KS18Volume.lean` |
