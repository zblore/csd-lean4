...
**Two endpoints — do not conflate them.** (1) *Operational finite-QM closure on a concrete projective product witness* — this is what the proofs deliver, essentially complete. (2) *A faithful derivation of the Paper C/D architecture from a primitive ontology* — **not** complete, and not claimed. The precise, defensible claim is: **the repository proves operational finite-QM closure on a concrete projective product witness satisfying the exact formalised subset of the Paper C assumptions.** The witness `(Σ = ℂℙ^{N-1} × T², μL = μFS ⊗ vol, Φ_t = (e^{-itH}·[p], θ), π = Prod.fst)` has the Fubini–Study measure and unitary evolution **built in**, so the pushforward-to-`μFS` and the Schrödinger pillar are *compatibility facts about the witness*, not derivations of projective geometry, `μFS`, or unitary dynamics from a more primitive ontic model.

**The two genuine open frontiers.**
- **SO-1 — the sector-origin problem.** Everything above is FORWARD — it consumes the posited Kähler sector. The Born trials *sample* the sector's measure i.i.d.; they are **not** *evolved by the deterministic flow*, and the Born weights are **not** *derived from a primitive ontology*. Closing this — deriving the sector `(Σ, π, μL)` and its typicality from `Φ` itself — is the sector-origin problem (manifest link **L7**), research-grade and open. **SO-1 is distinct from Paper C's Axiom A5**, which is the *quantum-effective / projectability* condition (`H = h∘π + δH`, `sup‖d(δH)|_V‖ ≤ ε`) that *selects* the sector — not its origin. Paper C §A/§3.6 explicitly assumes `Σ`, `π` and the A5 sector and leaves "the deeper physical origin of the quantum-effective sector selected by A5 … for later work", so this scope matches the paper's own.
- **MD-1 — the A7 measurement-partition mismatch.** Paper C A7 makes outcome regions context-fixed, `Ωᵢ = Ωᵢ(M)`. The present `vnPointerOutcome` uses `bornRegion ψ'`, so the cell *shapes* depend on the prepared/dilated state — a **preparation-indexed operational witness**, not the general A7 mechanism. Recorded as the next scientific tranche (MD-1), not redesigned here.
...
| Born weights derived FROM the flow (L7) | **OPEN ★** — the thesis frontier (SO-1, the sector-origin problem); the sector itself is posited |

**Current work programme:** the forward reconstruction (L2–L9) and the SigmaLayer T1–T16 target inventory are landed on the concrete witness; remaining work is documentation/CI hygiene, breadth, and the deep frontiers — **SO-1** (the sector origin), **MD-1** (the A7 preparation-indexed vs. context-fixed partition mismatch), the general Lindblad tier, and the Kähler 2-form. Details: [`specs/future-work.md`](specs/future-work.md) and the [connectivity manifest](specs/connectivity-manifest.md).
