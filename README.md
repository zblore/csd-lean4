...
| **Alg** (algorithms) | n-qubit register; Deutsch-Jozsa; Simon (period-promise, outcomes `⊥ s`); Grover `sin²((2k+1)θ)`; QFT unitarity; full Shor (quantum core + recovery + factoring + random-`a` ≥ 1/2 for arbitrary odd composite `N` + capstone) | `Mathlib/QuantumInfo/{Register,Hadamard,Fourier}.lean`, `Empirical/QM/Algorithms/{DeutschJozsa,Simon,Grover,ShorCore,ShorRecovery,ShorRandomA,ShorCapstone}.lean` |
...
CsdLean4/
  ...
  Empirical/
    QM/                -- Pure QM-validity content (no CSD ontology)
      Algorithms/      -- Deutsch-Jozsa, Simon, Grover, Shor (core + recovery +
