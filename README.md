...
| ONE model carries dynamics + measurement + records + Born + update (L9) | **CONNECTED** — `unified_projectiveSector_capstone` (FND-T5) |
| Born weights derived FROM the flow (L7) | **OPEN ★** — the thesis frontier (A5→D1, FND-1); the sector itself is posited |

**Current work programme:** the forward reconstruction (L2–L9) and the SigmaLayer T1–T16 ledger are landed; remaining work is documentation/CI hygiene, breadth, and the deep frontier (A5/FND-1, the general Lindblad tier, the Kähler 2-form). Details: [`specs/future-work.md`](specs/future-work.md) and the [connectivity manifest](specs/connectivity-manifest.md).
