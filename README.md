...
**The near frontier — the record layer (MD-1).** Measurement should be a **record selection via context-fixed Ω-basins on Σ**: a partition of the prepared region `Ω₀` determined by the apparatus, the record being *which basin the single trajectory occupies* (a selection of structure already in Σ — Σ does not grow). The corpus currently does measurement on the **epistemic** side — `vnPointerOutcome` via `bornRegion ψ'`, preparation-indexed on `ℂℙ` — the wrong side; completing the reconstruction moves it onto Σ. (The older "SO-1 = derive the sector" framing is a **retired error** — Σ is the floor; and Paper C **Axiom A5** = *projectability*, never "the origin of Σ".)
...
**Current work programme:** the QM calculation engine (L2–L9, the T1–T16 inventory) is landed on the concrete witness. The goal now is to **complete the reconstruction of QM from Σ and Ω-regions**; the near frontier is the **record layer** (**MD-1** — measurement as record selection via context-fixed Ω-basins on Σ, replacing the epistemic `bornRegion ψ'`). Other open items: the general Lindblad tier and the Kähler 2-form. Details: [`specs/CSD-CHARTER.md`](specs/CSD-CHARTER.md), [`specs/future-work.md`](specs/future-work.md), the [connectivity manifest](specs/connectivity-manifest.md).
