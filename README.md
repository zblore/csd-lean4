...
| **ECDLP / reversible arithmetic** | A **derived-cost** reversible-circuit stack toward Shor/ECDLP resource accounting on secp256k1 (`Mathlib/QuantumInfo/Reversible/`, `Mathlib/QuantumInfo/ECDLP/`): a gate-list DSL + cost model where cost is a *function of the exhibited gate list* (not annotated); **ModAdd** (`rippleCirc_correct`: `(A+B) mod 2ⁿ`); **ModMul** (`mulCircuit_correct` `Acc=a·Y` + `mulCircuit_correct_zmod` the `ZMod N` mulOracle action); **ModInv** (inverse oracle + `mulConst_modInv_bijective`); the **EC layer** (`scalarMul [k]P` wrapping `WeierstrassCurve.Affine.Point`, the `IsDLog`/`Solvable` ECDLP statement); **double-and-add** (`doubleAndAdd_eq_scalarMul` + `doubleAndAddCost_le ≤ 2·Nat.size k`); and the **secp256k1 capstone** `secp256k1_scalarMul_toffoli_bound` (`O(256³)·M` Toffolis, composing the O(log k) op count and the `2n²` multiplier). Plan: [`specs/ecdlp-resource-plan.md`](specs/ecdlp-resource-plan.md) | A **sorry-free resource scaffold** (the `O(n³)` Toffoli structure), **not** a verified attack: the concrete reversible EC-addition circuit (which would fix the field-mults-per-op parameter `M`), `p`-primality, and an on-curve point are named residue | foundational triple only on every pin; each of the 7 tranches independently auditor-SOUND |
...
| **V** (volume series) | Born numbers *derived* (not transported) as Fubini-Study volumes: Stern-Gerlach, Malus, Bell, GHZ, Hardy, the trine / USD / SIC / qutrit / Hesse / MUB POVMs, and **any projective measurement context**, rank-1 *and* degenerate (`context_born_frequency_volume` / `block_born_frequency_volume` — the general contextuality grounding, Kochen-Specker + Mermin-Peres rank-2 eigenspaces as block sums of FS volumes). Genericity-free since the 2026-06-11 hpos migration (`BornRegionUncond.lean`): boundary preparations — context eigenstates, the Mermin GHZ points `Φ = 0, π`, aligned Bell analysers — covered | `Empirical/CSD/{SternGerlach,Malus,Bell,GHZ,Hardy,Trine,USD,SIC,…}Volume.lean`, `Empirical/CSD/ContextVolume.lean` |
...
| **ECDLP** (reversible-arithmetic resource accounting) | Derived-cost reversible-gate DSL; ModAdd (`(A+B) mod 2ⁿ` correct); ModMul (`Acc=a·Y` + `ZMod N` mulOracle); ModInv (inverse algebra + reversibility link); EC `scalarMul [k]P` + ECDLP statement; double-and-add (O(log k) cost); secp256k1 capstone (`O(256³)·M` Toffoli scaffold). 7/7 tranches, each auditor-SOUND | `Mathlib/QuantumInfo/Reversible/{Circuit,Cost,ModAdd,ModMul,ModInv}.lean`, `Mathlib/QuantumInfo/ECDLP/{EllipticCurve,ScalarMul,Secp256k1,ResourceBounds}.lean` |
