...
**The near frontier — the record layer (MD-1).** Two levels, kept distinct (Paper C A7): the apparatus context M fixes an **epistemic** outcome partition **{Ωᵢ(M)} ⊂ ℂℙⁿ⁻¹** (the Ω-regions are *epistemic*, not ontic), and **the record is the ontic selection in Σ** of which region the single trajectory realizes (a selection of structure already in Σ — Σ does not grow). The corpus's gap is *not* "move regions onto Σ": (1) `vnPointerOutcome` uses `bornRegion ψ'` — **preparation-indexed** (from the state), not **context-fixed** {Ωᵢ(M)} (from the apparatus M), both epistemic; and (2) the record is not yet realized as the ontic selection. (The older "SO-1 = derive the sector" framing is a **retired error** — Σ is the floor; Paper C **Axiom A5** = *projectability*, never "the origin of Σ".)
...
**Current work programme:** the QM calculation engine (L2–L9, the T1–T16 inventory) is landed on the concrete witness. The goal now is to **complete the reconstruction of QM from Σ and Ω-regions**; the near frontier is the **record layer** (**MD-1** — context-fixed *epistemic* outcome regions {Ωᵢ(M)} on `ℂℙⁿ⁻¹`, replacing the preparation-indexed `bornRegion ψ'`, with the record the ontic selection in Σ). Other open items: the general Lindblad tier and the Kähler 2-form. Details: [`specs/CSD-CHARTER.md`](specs/CSD-CHARTER.md), [`specs/future-work.md`](specs/future-work.md), the [connectivity manifest](specs/connectivity-manifest.md).
