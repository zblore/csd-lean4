...
|---|---|---|---|
| **LF5 measurement dynamics (in progress)** | The D1 frontier (`specs/lf5-plan.md`): LF5-A `vnUnitary` — the von Neumann coupling permutation unitary (copy `eⱼ⊗a₀ ↦ eⱼ⊗aⱼ`, manifestly unitary); LF5-B `measurementFlow` — the **first measurement flow `Φ_vN ≠ id`** on the dilated `ℂℙ^{N·N−1}`, Fubini–Study measure-preserving (`measurementFlow_measurePreserving`, `measurementFlow_ne_id`, `measurementFlow_mk_single`). LF5-C/D/E (dynamical Naimark dilation, Born = volume + frequency on the flow, capstone) open | **No carving** — pointer regions will be the context-fixed `blockProj` apparatus blocks; the Born number is reused from the FS-volume engine (not re-derived), per the de-isolation reading (carve-out-plan §6) | none |
