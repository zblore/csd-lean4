...
**The engine vs. the reconstruction (frame: [`specs/CSD-CHARTER.md`](specs/CSD-CHARTER.md)).** What the proofs deliver is the **QM calculation engine** on a concrete projective *witness*. The witness `(Σ = ℂℙ^{N-1} × T², μL = μFS ⊗ vol, Φ_t = (e^{-itH}·[p], θ), π = Prod.fst)` has μ_FS and unitary evolution **built in**, so its `π_*μL = μFS` and `exp(-itH)` facts are *compatibility facts about the witness*, not derivations of projective geometry, μ_FS, or unitary dynamics from anything more primitive. **Σ is the floor and is everything — deriving it is a non-question** (Paper C is explicitly "a reconstruction, not a derivation"). The goal now is to **complete the reconstruction of QM from Σ and Ω-regions** — QM genuinely *arising from* the ontic surface, not accumulated on the epistemic side.

**The record layer (MD-1) — the KINEMATIC interface is BUILT (2026-07-25); MD-1 is NOT discharged.** ⚠️ *Scope corrected 2026-07-28 after an external review. What is proved is that, **given** a basin partition, an unknown microstate selects one definite record and repeated i.i.d. preparations obey the LLN. Three things are open: **general-`N` A7** (the partition is built from the preparation, so `Ωᵢ(M)` fixed by the apparatus alone is established only at `N=2`, `LF4/QubitBorn.lean`); the **de-isolation dynamics** (`DeIsolationInteraction.basin_rate` is a hypothesis field — no `H_int` generating the basins is constructed); and the **A1-sector status** of the fibred Σ (`ℂℙⁿ⁻¹ × ℝ` with restricted Lebesgue — non-compact fibre, no Kähler structure, measure not shown Liouville). See `specs/BACKLOG.md`.* The measurement layer is formalized end-to-end in Lean (`specs/record-layer-plan.md §4`), foundational-triple, no `sorry`, axiom-pinned. The honest CSD ontic space is **Σ = base × fibre** (`SigmaLayer/FibredSigma.lean`): the **base** `ℂℙⁿ⁻¹` is the *epistemic* projective point (pinned to `[ψ]` for a sharp prep), the **fibre** carries the *ontic* record coordinate. A measurement is **`context + unknown microstate → record`** (`SigmaLayer/Measurement.lean`): the context fixes the basin partition, the unknown microstate selects the basin it occupies, the combined result is a genuine postulate-P5 `RecordSemantics` record. The outcome probabilities are the **Kähler torus moment map** (`SigmaLayer/MomentMapRace.lean` — forced by the geometry, not injected), and the Born rule is nothing but the **law of large numbers over the unknown initial microstate** (`Measurement.bornMeasurement_frequency` — randomness = ignorance of the initial condition; no dynamical postulate). This is realized on the corpus's actual measurement space: `KSigmaRecord.born_frequency_region_eq_record` proves the region `FiniteQMClosure.born_frequency` lands in *is definitionally the record-layer event*. (The older "SO-1 = derive the sector" framing is a **retired error** — Σ is the floor; Paper C **Axiom A5** = *projectability*, never "the origin of Σ".)
...
| Born weights derived FROM the flow (L7) | **N/A** — the sector itself is posited (Σ is the floor; deriving it is a non-question). Born arises from the Ω-region volume ratios + typicality, which is the reconstruction, not a "derive Σ" gap. |

**Current work programme:** the QM calculation engine (L2–L9, the T1–T16 inventory) is landed on the concrete witness, and the **record layer (MD-1)** that completes the reconstruction of QM from Σ and Ω-regions is now **built** (2026-07-25, `specs/record-layer-plan.md §4`): measurement = `context + unknown microstate → record` on the base×fibre Σ, outcome probabilities = the Kähler moment map, Born = the law of large numbers over the unknown microstate. Remaining items are optional/mechanical (a field naming in the pinned closure, carrying no new theorem) or unrelated open tiers (the general Lindblad tier, the Kähler 2-form). Details: [`specs/CSD-CHARTER.md`](specs/CSD-CHARTER.md), [`specs/future-work.md`](specs/future-work.md), the [connectivity manifest](specs/connectivity-manifest.md).
...
| **E** (QI / crypto) | No-broadcasting, no-communication, teleportation, Robertson uncertainty, E91 device-independent security, unambiguous discrimination (USD), the Helstrom minimum-error bound | `Empirical/QM/{NoBroadcasting,NoCommunication,Resources/Teleportation,Uncertainty,Crypto/E91,USD}.lean`, `Mathlib/QuantumInfo/Helstrom.lean` |
