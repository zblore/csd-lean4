...
**Both load-bearing pillars of quantum mechanics — the Born rule and the Schrödinger equation — are machine-verified here as theorems of a single posited object.** Posit a compact Kähler ontic sector: a substrate `Σ` carrying a deterministic, Liouville-volume-preserving flow, a projection onto ray space `ℂℙ^{N-1}`, and probability read as typicality volume. From that object — and nothing else beyond named, explicitly-hypothesised regularity data — this Lean 4 formalisation of Constraint-Surface Dynamics derives, sorry-free and Gleason-free:

- **the Born rule** — for every dimension, every unit preparation, and *general* (POVM) measurements: empirical frequencies of deterministic repeated trials converge a.s. to Fubini–Study volumes, which *are* the Born weights (LF1–LF4);
- **the Schrödinger equation** — the projected sector flow is *forced* to be `exp(-itH)`-conjugation on rays with Hermitian `H` recovered from the flow (the W-series, complete 2026-07-07);
- **measurement as a process** — a genuine `Φ ≠ id` von Neumann de-isolation flow realises the Naimark dilation dynamically, with per-microstate deterministic outcomes (LF5);
- **entanglement, non-locality, no-signalling** — Bell-forced non-factorisation in the `Σ`-engine at full generality on both axes: CGLMP violation for *every* dimension `d`, GHZ/Mermin for *every* party number `n` (LF6);
- **uncertainty** (Robertson at the ontic-integration level, LF4 §14.2) and **thermal equilibrium in expectation** (canonical typicality, TH1).

**The pillar ledger** — what is derived, what is posited, what is open:

| QM pillar | Status |
|---|---|
| Born rule (incl. POVMs) | **Derived** (Gleason-free, unconditional, every `N`) |
| Unitary / Schrödinger dynamics | **Derived** from the sector, on named regularity data (FS-isometry, Bargmann-observable continuity, phase coboundary, C¹) |
| Measurement dynamics & outcomes | **Derived** (single-system projective tier complete; entangled tier exercised) |
| Uncertainty (Robertson) | **Derived** (ontic-side variances) |
| Entanglement / non-locality / no-signalling | **Derived** (general `d`, general `n`) |
| Statistical mechanics | **Substantially covered** — TH1 canonical typicality (expectation) + TH2 second law (H-theorem) + TH3 free energy (Gibbs variational principle) + TH4 Landauer (`kT ln 2`) all done; ETH / fluctuation theorems (TH5) the optional stretch |
| Continuous variables (`Q`, `P`, CCR, `ℏ`) | **Open** (CV-1..4 planned; the finite-dim CCR obstruction W4 is done) |
| Tensor-product / composite structure | **Posited** per instance; the derivation (P3) is parked pending a paper proof |
| Identical particles / spin-statistics | **Not addressed** (named deferral, IP-1; presupposes P3) |
| The sector itself (why `ℂℙ^{N-1}`, why `μ_FS`, why `ℂ`) | **The open frontier** — A5→D1 (FND-1): everything above is conditional on the posited sector; deriving it from the dynamics is the one thesis-critical residue |

**Current work programme, in order** (set 2026-07-07): the **thermodynamics track** (TH-2 second law → TH-3 temperature/free energy → TH-4 Landauer, plus the TH-1 Levy-concentration residual), then **continuous variables** (CV-1 onwards); the ECDLP/EC-6 artifact track is deprioritised behind both. Details: [`specs/future-work.md`](specs/future-work.md).
