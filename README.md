...
|---|---|---|---|
| **LF6 entangled de-isolation tier (first results 2026-06-28)** | The non-local strata of D1. **LF6-A:** `no_product_partition_realises_singlet` — the singlet's correlations cannot come from a product/factorised partition of the `Σ`-engine (non-factorisation is **Bell-forced**, derived not posited; factorisation would itself be a measurement), with the contextuality corollary; `singletDeisolationFlow` — a genuine `Φ ≠ id` de-isolation flow on the dilated `ℂℙ¹⁵` realising the singlet dilation. **LF6-B:** the decoherence / partial-trace tier (D1b) — a purity-drop / irreversibility witness on the reduced state. Honest scope: single-instance (the singlet) entangled landings; the **general-`N` entangled tier remains open** (#5) | **No carving** — the non-factorisation is derived from the engine structure; the flow realises (not carves) the dilation | none |
