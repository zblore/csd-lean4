...
| Axiom | Role | Source |
|---|---|---|
| `invariant_measure_uniqueness` | `G`-invariant probability measure on `P` is unique (Fubini–Study in the concrete CSD model) | Spec-mandated; not in Mathlib |
| `busch_effect_gleason` | Effect-additive probability on finite-dim `N ≥ 2` admits a unique trace-form density | Spec-mandated; not in Mathlib |
