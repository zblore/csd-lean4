...
**The posit and the near frontier.** The sector `(Σ, π, μL)` + Fubini–Study typicality is **POSITED** — **Σ is the floor and is everything; deriving it is a non-question** (Paper C is explicitly "a reconstruction, not a derivation"; μ_FS is fixed by SU(n) symmetry, Paper B; and Paper C **Axiom A5** = the *quantum-effective / projectability* condition on Hamiltonians, `H = h∘π + δH`, `sup‖d(δH)|_V‖ ≤ ε`, that *selects* the sector — **never** "the origin of Σ"). The goal is to **complete the reconstruction of QM from Σ and Ω-regions**; the near frontier is **MD-1 — the record layer:** make measurement a record *selection* via context-fixed Ω-basins on Σ, replacing the present preparation-indexed `bornRegion ψ'` on `ℂℙ` (an epistemic operational witness, not the context-fixed `Ωᵢ(M)` of Paper C A7). (The earlier "SO-1 = derive the sector" framing here was a retired error — Σ is the floor.)
