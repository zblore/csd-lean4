...
**The single remaining deep posit, unchanged by the Σ-layer:** the sector `(π, G)` + Fubini–Study typicality is POSITED, not derived from the deterministic flow (A5 → D1, tracked as `SL-1` in `specs/future-work.md`; the connectivity-manifest link L7). Everything above is FORWARD (consumes the posited sector); the Σ-layer organises and unifies it under a clean ledger and does not close A5.
