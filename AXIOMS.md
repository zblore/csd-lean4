...
**So `#print axioms` measures categories 1 and 2 (Lean + library), not category 3 (the physics).** With category 2 now **empty**, `#print axioms` on every corpus theorem reports only the foundational triple — a statement about library completeness, *not* about what CSD commits you to. The figure that describes the theory is **category 3**: the ontic substrate + the sector posit + typicality, with the **Born rule now a theorem** of those (Gleason-free, general dimension, including general POVMs) rather than a postulate.

⚠️ **"Gleason-free" means one thing only:** that the Σ-side road shares no machinery with Gleason or Busch — a *provenance* claim, machine-checked by two guards — `check-import-negative.sh` (46 modules: `LF2/EffectGleason` absent from the import closure) and `check-gleason-free.sh` (11 modules, 259 declarations, none reaching `effect_gleason_representation` in the constant graph). It does **not** mean the Born form is derived from nothing. What characterises the cell law is torus generation. The corpus's own cell-law characterisation is **single-context** (`torusGenerated_eq_momentMap`), so it routes through neither and imports no noncontextuality. ⚠️ That is not a claim that no Gleason-style characterisation exists: the frame-function route was **declined, not refuted** (`specs/cell-law-scoping.md`, stage 2). The structural posits themselves — what the theory asks you to accept, and what would discharge each — are registered in [`specs/POSITS.md`](specs/POSITS.md). Categories 1 and 2 are infrastructure — the triple is unavoidable, and there is no longer any imported library gap.
...
**This is the category that describes the theory** (see §0, category 3): what CSD asks you to accept. The Born rule is *not* here — it is a theorem of these postulates (Gleason-free in the provenance sense of §0, general dimension, general POVMs), given the cell law of §3.10. The postulates are carried as structural data / hypotheses on the types rather than as `axiom` declarations, because CSD's claims are conditional on the ontic substrate existing with these properties; so they do not appear in `#print axioms`. The load-bearing ones are the **ontic substrate** (§3.1, §3.2: a measure space `(Σ, μL)` with a deterministic, measure-preserving flow `Φ`), the **sector posit** (§3.3: the projection `π` onto the quantum-effective sector and its symmetry `G`), and the **typicality reading** (probability *is* `μL`-volume ratio, the interpretive posit that LF1's repeated-trial law encodes and the strong law shows self-consistent). ⚠️ *Restated 2026-09-05 — the phrase below ran three different things together; `specs/POSITS.md` splits them, and "deriving the outcome regions from the dynamics" is now the trichotomy's three cases, not one open item.* The deepest open item is the **dynamical origin of the sector**: deriving `π` / the outcome regions / `Φ` from the dynamics rather than positing them (the LF5 measurement flow `Φ_vN ≠ id` is exercised at the single-system projective tier as of 2026-06-11, but the concrete `SectorData` instances still carry `Φ = id` and the sector itself remains posited). *Addendum 2026-08-02:* the **measurement dynamics itself is now constructed, not posited** — explicit measure-preserving propagators create, persist, and Born-weight records and implement the Lüders update (rank-one: the calibrated swap; degenerate: the projective-join unitary), with the honest classification that these propagators are **piecewise rigid *symplectic* translations with a provably-forced null seam set** (`ShearDiscontinuity`/`PiecewiseHamiltonian`) — ⚠️ *corrected 2026-08-04: this line read "piecewise Hamiltonian" long after `PiecewiseHamiltonian.lean`'s own header withdrew that reading on the torus-flux obstruction (`ι_Xω = a·dp` is closed but not exact on `T²`, so no global generator exists). A genuinely Hamiltonian-generated propagator arrived later by a different route — the projective pointer `smoothWitnessClosure`/`rampedU_schrodinger`, where `H¹(ℂℙ^K) = 0` kills the obstruction*; the calibration itself is a posit, recorded as §3.8 below. These are honest assumptions about which class of mathematical objects the corpus is talking about; they are not derived inside the Lean tree.
...
**The posit and the near frontier.** ⚠️ *Frontier vocabulary superseded 2026-09-05:* `specs/POSITS.md` ("What 'frontier' means here") separates **permanent boundary** / **open mathematics** / **open foundations**. The surviving foundational frontier is Posit 1's discharge condition — deriving `IsTorusGenerated` from the de-isolation dynamics. The extensions are breadth, not the frontier.  The sector `(Σ, π, μL)` + Fubini–Study typicality is **POSITED** — **Σ is the floor and is everything; deriving it is a non-question** (Paper C is explicitly "a reconstruction, not a derivation"; μ_FS is fixed by SU(n) symmetry, Paper B; and Paper C **Axiom A5** = the *quantum-effective / projectability* condition on Hamiltonians, `H = h∘π + δH`, `sup‖d(δH)|_V‖ ≤ ε`, that *selects* the sector — **never** "the origin of Σ"). The goal is to **complete the reconstruction of QM from Σ and Ω-regions**; the **record layer** (MD-1) that does this is now **formalized in Lean** (2026-07-25, `specs/record-layer-plan.md §4`): measurement as `context + unknown microstate → record` on the base×fibre Σ, Born = the law of large numbers over the unknown microstate, outcome probabilities = the Kähler moment map (`SigmaLayer/{Measurement,MomentMapRace,FibredSigma,ProjectiveRecord,OnticBornFrequency}.lean`). The residual MD-1 item is context-fixing the *pinned closure's* regions (still the preparation-indexed `bornRegion ψ'`); the genuine near frontier is now the **extensions** — continuous spectra (CV), relativistic locality, identical particles. (The earlier "SO-1 = derive the sector" framing here was a retired error — Σ is the floor.)
...
### 3.10 The cell law (outcome rates) — *added 2026-09-05, characterised 2026-09-04*

`RecordLayer/GlobalBasin.lean` defines `momentContext : ContextField N`, whose rate is the
Fubini–Study torus moment map, and `globalBasin_prob` holds for **every** `ContextField` — so the
choice of that field is a commitment, and it was missing from this ledger.
...
⚠️ **Restated, not discharged.** `IsTorusGenerated` is extensionally equivalent to the conclusion,
so this is a characterisation rather than a derivation, and the posit count is unchanged. What
improves is the premise: it mentions no probability, no amplitude and no Born target, so the Born
*shape* of the rates is no longer assumed in order to derive Born. The manifold form of the
moment-map equation is posited (author decision 2026-09-04; Mathlib has no symplectic manifold API).
Discharging it means deriving generation from the de-isolation dynamics — the `H_int` frontier.
...
| `Projectivization.continuous_iff` | `propext, Classical.choice, Quot.sound` |
