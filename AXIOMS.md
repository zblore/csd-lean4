...
**The two remaining deep gaps, unchanged by the Σ-layer.** (1) **SO-1 — the sector-origin problem:** the sector `(Σ, π, μL)` + Fubini–Study typicality is POSITED, not derived from a primitive ontology / deterministic flow (tracked as `SO-1` in `specs/future-work.md`; the connectivity-manifest link L7). This is **distinct from Paper C's Axiom A5**, which is the *quantum-effective / projectability* condition on Hamiltonians (`H = h∘π + δH`, `sup‖d(δH)|_V‖ ≤ ε`) that selects the sector — Paper C §3.6 itself leaves the sector's deeper origin "for later work". Everything above is FORWARD (consumes the posited sector); the Σ-layer organises and unifies it and does not close SO-1. (2) **MD-1 — the A7 measurement-partition mismatch:** the present outcome cells (`bornRegion ψ'`) are preparation-indexed, not the context-fixed `Ωᵢ(M)` of Paper C A7 — a preparation-indexed operational witness, recorded (not redesigned) as the next tranche.
