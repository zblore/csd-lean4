...
2. **Imported mathematical results** (§2) — standard theorems (invariant-measure uniqueness; the Busch effect-Gleason theorem) that are not yet in Mathlib, so the formalisation imports them as `axiom`. **These are formalisation debt, not commitments of the physical theory** — each is provable in principle and eliminable as the library grows. One of the two is already eliminable: its concrete content is a proved, axiom-free theorem in the tree (§2.1).
