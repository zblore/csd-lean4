...
| Wigner–Araki–Yanase | `WignerArakiYanase.lean` | `arakiYanase_identity`, `wigner_araki_yanase`, `no_exact_record_of_not_commute`, `way_hypotheses_satisfiable`, `swap_exact_record_not_commute`, `sigmaX_no_exact_conserving_record` | A measurement isometry `U` on `HS ⊗ HA` conserving an additive `L = L_S ⊗ 1 + 1 ⊗ L_A` that records the eigenbasis of `A` **exactly** (orthogonal pointers, Yanase condition on the pointers *or* repeatable post-states) forces `[A, L_S] = 0`; hence no exact conserving record of `A` when `[A, L_S] ≠ 0`. Non-vacuous (CNOT records `σ_z` exactly while conserving `σ_z ⊗ 1 + 1 ⊗ σ_x`; its pointer fails Yanase, repeatability saves it) and sharp (SWAP records `σ_x` exactly while conserving `σ_z ⊗ 1 + 1 ⊗ σ_z` with `[σ_x, σ_z] ≠ 0` — both side conditions fail, so the side condition is load-bearing). The record-layer stroke sits *outside* these hypotheses (CSD twin `CSD/WignerArakiYanase.lean`, `no_joint_hilbert_map`); the LF5 coupling `vnUnitary` meets them and WAY's conclusion holds there (`specs/way-theorem-scoping.md`) | Wigner 1952; Araki–Yanase 1960; Yanase 1961; Ozawa 2002 (quantitative form: not formalised) |
...
| Quantum eraser (CSD twin) | `CSD/QuantumEraserVolume.lean` | `eraser_fringe_typicality`, `eraser_dark_typicality_zero`, `eraser_dark_record_null`, `eraser_dark_measurement_zero`, `eraser_dark_basin_null`, `eraserOut_rate_conditional`, `mk_eraserOut_pi` | The marker-conditioned fringe `(1 + c·cos φ)/2` as a fibre-typicality volume at **every** phase — and at `φ=π` the **dark fringe is an ontic impossibility**: the dark cell, the P5 record event, the measurement probability, *and* the v1.0 context-fixed basin are all exactly null (at the dark point the conditioned state *is* the vertex `[e₁]`). Conditioned rates derived as joint/marginal from the QM module. Stated via the **record layer** (like the HOM twin): the DH route's `hpos` excludes exactly these zeros. ⚠️ *Caveat discharged 2026-08-03*: the conditioning **process** is now realised — `CSD/EraserDynamics.lean` derives the conditioned states from the local Lüders strokes themselves (`erased_amp`: the dynamical post-states are `√2·eraserOut`; `marked_no_fringe`; `erased_weight`), and `CSD/EraserSequential.lean` adds `sequential_no_revival` — erasing after the mark record revives nothing: records are statistically irreversible | — |
| Quantum eraser | `QuantumEraser.lean` | `eraser_joint`, `eraser_no_interference`, `eraser_fringe_bright`, `eraser_fringe_dark` | Complementarity + erasure: joint `P(a,c) = (1 + ac·cos φ)/4` — full-visibility fringe in the marker-conditioned statistics, yet the unconditioned system marginal is flat `1/2` (which-path information kills interference; erasing it restores fringes only in the conditioned subensembles; delayed choice = same statistics). ⚠️ *Row added 2026-08-02: the module landed earlier, wired and pinned, but this table row was missing* | Scully–Drühl 1982; Kim et al. 2000 |
...
| B92 sequential (CSD twin) | `CSD/Crypto/B92Sequential.lean` | `b92_honest_false_click_null`, `b92_honest_false_click_null_z`, `b92_conclusive_basin_half`, `b92_eve_false_click`, `b92_eve_detectable` | **Unambiguity as a null basin**: an honest `\|+⟩` carrier has a zero-width conclusive-bit-0 arc (no microstate can produce a false click — the eraser-dark-fringe shape); Eve's Z-intercept (calibrated-swap dynamics) raises the false-click basin to exactly `½` whatever she recorded, strictly above the honest zero — eavesdropping announces itself in the conclusive statistics. Instantiates the `BB84Sequential` engine (stated, not hidden) | — |
| Wiesner money sequential (CSD twin) | `CSD/Crypto/WiesnerSequential.lean` | `wiesner_honest_x_pass`, `wiesner_honest_z_pass`, `wiesner_forge_x_pass_half`, `wiesner_forge_x_caught_half`, `wiesner_forge_z_invisible`, `wiesner_rate_eq_verifyProb`, `wiesner_forge_pass_avg` | The counterfeit round as a genuine sequential measurement: honest verification's pass basin has **full** measure (`wiesner_rate_eq_verifyProb` ties the rate to the QM `verifyProb`); the forger's measure-resend on a conjugate-basis note leaves the bank's pass/reject basins at exactly `½` each (collapse = pushforward theorem), and on a matching-basis note is repeatability (invisible — the secret basis *is* the security); the `¾` per-position average is the `(3/4)ⁿ` counterfeiting value, **attained** by measure-resend (optimality per Molina–Vidick–Watrous out of scope) | — |
| BB84 sequential (CSD twin) | `CSD/Crypto/BB84Sequential.lean` | `bb84_wrong_basis_bob`, `bb84_wrong_basis_error`, `bb84_right_basis_no_disturbance`, `bb84_right_basis_faithful`, `bb84_eve_selector_born`, `bb84_dynamical_matches_marginal`, `xContext_rate_vertex` | Intercept-resend with the **collapse step a theorem**: Eve's measure-and-resend is `csd_sequential_born` (calibrated-swap dynamics), not a classical marginal — dissolving the QM module's own "LF5 gate" scope note. Wrong basis: Bob's basins are exactly `½` whatever Eve saw (Bob reads the **rotated X-context** `basisContext xBasisON` — both measurements are context-field reads, end-to-end dynamical). Right basis: exactly repeatability (error basin null). `hpos` proved from the preparation (`prep_outcome_pos`), not carried. Dual round of the QM module's; the values assemble to the same ¼ QBER | — |
| BB84 QKD (intercept-resend) | `Crypto/BB84.lean` | `bb84_qber`, `bb84_intercept_resend_right_basis`, `bb84_intercept_resend_wrong_basis`, `bb84_eavesdropping_detectable`, `bb84_states_nonorthogonal` | The ¼ sifted-key QBER of intercept-resend eavesdropping: matching basis injects `0` error, wrong basis `½`, average `¼ > 0` — detectable; rooted in `⟨0\|+⟩ ≠ 0`. Eve's collapse modelled as a classical marginal (see the CSD twin row above for the dynamical upgrade). ⚠️ *Row added 2026-08-02: the module landed earlier, wired and pinned, but this table row was missing* | Bennett–Brassard 1984 |
| B92 QKD (two-state) | `Crypto/B92.lean` | `b92_nonorthogonal`, `b92_unambiguous_one`, `b92_unambiguous_zero`, `b92_conclusive_rate_one` | The minimal QKD protocol: two non-orthogonal states; Bob's unambiguous-discrimination readout (zero-error conclusive clicks at rate `½`); security root is the same non-orthogonality as Wiesner money. ⚠️ *Row added 2026-08-02, as above* | Bennett 1992 |
...
| Simon | `Algorithms/Simon.lean` | `simon_orthogonal`, `simon_uniform`, `simon_amplitude` | Simon orthogonality: after `Hⁿ` on the coset superposition `(\|x₀⟩+\|x₀⊕s⟩)/√2`, every measured `y` is orthogonal to the hidden period `s` (Born prob `0` when `⟨s,y⟩` odd; equal value `2/2ⁿ` on `s^⊥`). Single-register reduced analysis | Simon 1994 |
...
| QEC: 3-qubit code | `QEC/ThreeQubit.lean` | `csd_three_qubit_corrects_single_bitflip`, `bitFlipFlow_traceRight_barycenter`, `errorRegion_disjoint` | First QEC reading: codespace = sub-surface of `Σ`; error = decoherence (system→environment volume flow, Liouville-conserved jointly); syndrome measurement = the entropy-extraction that re-concentrates the system; weights = `Σ`-volumes. **Proved 2026-09-11 (W11):** the bit-flip channel is the environment marginal of a `Σ`-flow (concretely `bitFlipFlow` on `cpSectorData` over `ℂℙ³`, no lift hypothesis), the code space is a region of `Σ` whose four error images are pairwise disjoint and returned by recovery; the former unused bundle binder is gone. Not a theorem: the syndrome-conditioned recovery as one channel on the mixed state; the joint flow on register ⊗ environment. |
...
| Einselection follow-up (15a+) | `Einselection.lean` | `einselection_degenerate_boundary`, `einselectionN`, `einselectionN_degenerate` | The pointer-basis **selects-vs-degenerates** boundary: einselection selects the computational basis iff `p₀≠p₁` (`decohere_hadamard_offDiag_ne_zero_iff`), and DEGENERATES at `p₀=p₁` (reduced state `½I`, basis-invariant under every unitary). General-N dephasing (`decohereReducedN`, non-vacuous via `decohereReducedN_acts_nontrivial`), degenerate locus all-equal `⇒ (1/N)I`. Qubit channel derived (partial trace of the LF5 dilation); general-N posited-and-bridged. Ontic einselection-from-Σ-dynamics origin D1-gated. |
| Weak / unsharp measurement (15c) | `WeakMeasurement.lean` | `weak_born_unsharp_interpolation`, `weak_born_frequency_volume` | One-parameter unsharp qubit POVM `weakEffect±(η) = ½(I±ησ)` interpolating no-measurement (η=0, `½I`, no info — the 15a degeneracy) ↔ projective carve (η=1, sharp `|0⟩⟨0|`, full Born `‖ψ₀‖²`); η-bound load-bearing (PSD fails η>1), genuine partial information (`weak_partial_information_witness`, ¾ at η=½). **Volume reading** (full theorem, Gleason-free, unconditional): the weak Born weight = a sum of FS volumes on the ℂℙ³ Naimark dilation (`weak_born_frequency_volume` via `povm_born_frequency_volume_uncond`) — the "partial Σ-volume nudge". **Gated:** continuous measurement as a dynamical `Φ≠id` weak flow (D1 / entangled tier). |
| Quantum Zeno (15d) | `QuantumZeno.lean` | `zeno_survival_quadratic`, `zeno_survival_slope_zero`, `zeno_freezing`, `zeno_nonvacuous` | A watched system does not evolve: frequent projective re-measurement onto `|0⟩` freezes the state. **Mechanism DERIVED** (concrete `σx`/`|0⟩` witness): variance `(ΔH)²=1` computed from the matrices (`varH_eq`), the **quadratic short-time bound** `P(s)≥1−(ΔH)²s²` (`zeno_survival_quadratic`, from `cos²=1−sin²≥1−s²`; the coefficient is the computed variance, not a hypothesis), and the **zero initial slope** `P'(0)=0` (`zeno_survival_slope_zero`, via `HasDerivAt`). **Limit:** `P_n=P(t/n)^n≥1−(ΔH)²t²/n` (Bernoulli, `zeno_survival_lower_bound`) and `P_n→1` (`zeno_freezing`, full squeeze). **Non-vacuity:** `(ΔH)²>0` with full free decay `P(π/2)=0` (`zeno_nonvacuous`), so the freezing is non-trivial. The closed-form `exp(-isσx)=cos s·I − i sin s·σx` is the asserted standard qubit rotation; all else derived. **CSD reading:** each measurement re-carves the Σ-volume onto the initial sector. **Gated:** the dynamical measurement-interspersed Σ-flow `Φ≠id` (D1 / LF6). |
| Channel capacities (15e) | `ChannelCapacity.lean` | `dephasing_classical_vs_quantum`, `holevo_classical_eq_log_two`, `dephasing_destroys_coherence` | The de-isolation / dephasing channel `Φ_deph = decohereReducedN` (15a) on the K1-A von Neumann entropy: CLASSICAL info survives, QUANTUM coherence destroyed. **Classical-yes:** computational-basis states are FIXED POINTS `Φ_deph(|i⟩⟨i|)=|i⟩⟨i|` (`dephasing_fixes_basis_state`, general N); single-letter Holevo χ of `{(½,|0⟩⟨0|),(½,|1⟩⟨1|)}` = `log 2` (`holevo_classical_eq_log_two`, `S(½I)−½·0−½·0`), a full classical bit. **Quantum-no:** `Φ_deph(|+⟩⟨+|)=½I` (`dephasing_plus_eq_half_one`), entropy jump `0→log 2` (`dephasing_destroys_coherence`). `S(½I)=log 2` DERIVED via `vonNeumannEntropy_const_smul_one` (charpoly route, `spectral_sum_eq_of_charpoly_prod`); `S(pure)=0` from `vonNeumannEntropy_eq_zero_of_pure`. **Scope:** single-shot Holevo / coherent-information, NOT the regularized capacity; the general `χ≥0` bound (entropy concavity) is gated on the open SSA fork; the ontic Σ-volume capacity (`Φ≠id` throughput) D1-gated to LF6. CLOSES the open-system 15-series (15a-e). |
| Wigner–Araki–Yanase (record-layer twin) | `WignerArakiYanase.lean` | `record_not_factor_hilbert`, `no_joint_hilbert_map`, `record_mem_recordRegion_of_register`, `pointerEvolve_record_not_factor_hilbert`, `jointLift_record_not_factor_hilbert` | At the `IsJointLift` level the pointer image is not a function of the Hilbert data `(base ray, pointer)`: from identical data the fibre register coordinate selects any outcome of rate `≥ 2ε` (`IsJointLift.landing`), and distinct outcomes land in disjoint record regions — so no map on any joint Hilbert space `HS ⊗ HA` reproduces the stroke's pointer image from the Hilbert data, and WAY's hypothesis is met by no joint lift (the quantifier is a robustness one — `pointer_eq` pins every lift's pointer image to the witness's — not a WAY-strength one). A **scope** statement, not an escape: no conservation law is modelled at the record layer (`R-015`); the `ε`-Born statistics are untouched. Non-vacuous at `[(1,1)]` for `0 < ε ≤ ¼`; instances for the fibrewise witness and the back-reacting `jointLift` (every shift) |
...
| KCBS pentagon volume (`Contextuality/KCBSVolume.lean`, **2026-08-02**) | `kcbs_context_born_frequency_volume` (+ `_canonical`), `kcbs_pentagon_weight` | The representative pentagon context `{kv 0, kv 1}` completed by the cross product (Lagrange identity for unit norm), complexified via the `c3_inner` transport — orthonormality pulled from the QM side (`kv_orth`, `kv_unit`), nothing re-proved. Every ray's context Born weight = FS typicality volume on `Σ = ℂℙ²`; at the apex the ray-0 weight is the pentagon number `1/√5`, whose five-fold sum `√5` violates the noncontextual bound `2` (`kcbs_quantum_violation`). One representative context (KS18 discipline); the other four are identical instantiations under `kv_orth` | Klyachko–Can–Binicioğlu–Shumovsky 2008 |
