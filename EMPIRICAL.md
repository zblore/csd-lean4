...
| Any projective context (**contextuality**) | `ContextVolume.lean` | `context_born_frequency_volume`, `context_born_eq_rotated` | an **arbitrary** orthonormal measurement context `B`: the outcome Born weights `‖⟨Bᵢ,ψ⟩‖²` are FS typicality volumes, by reducing to `born_frequency_convergence_N` on the `B.repr`-rotated state. The general grounding for projective contextuality (Kochen-Specker; rank-1). Honest scope: rank-1 (non-degenerate) contexts only; the context-dependence is realised as frame-dependence of the carved regions (`Φ = id`) |
