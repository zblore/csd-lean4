...
| QEC: 3-qubit code | `QEC/ThreeQubit.lean` | `csd_three_qubit_corrects_single_bitflip` | First QEC reading: codespace = sub-surface of `Σ`; error = decoherence (system→environment volume flow, Liouville-conserved jointly); syndrome measurement = the entropy-extraction that re-concentrates the system; weights = `Σ`-volumes. ⚠️ *Read exactly (2026-09-11):* the theorem binds its `CSDThreeQubitBundle` as `_b`, an **unused** argument, and its statement is the QM theorem's — the ontic reading in this cell is prose (the module's own TRANSPORT-ONLY tag). Making it a theorem is W11 of `specs/qit-chain-scoping.md`. Honest obligation = CPTP channels + `Φ≠id`. |
