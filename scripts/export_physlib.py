...
PROVENANCE_RE = re.compile(r"\*\*Provenance( and references)?\.?\*\*|## Provenance|Provenance:")
...
def hygiene_hits(src: str) -> list[tuple[int, str, str]]:
    """Hygiene hits outside provenance text, as (line number, category, line). Provenance text
    is a `## Provenance` section of a module docstring (to the next `## ` heading or the closing
    `-/`) or an inline `**Provenance.**` paragraph (to the next blank line)."""
...
def manifest(deps, lines, slices, target, phys_rev, local_rev, hyg, build_results, sweep_text):
    ...
    L.append(f"Lines mentioning CSD-specific vocabulary (`CSD.`, `LF1`–`LF6`, `ontic`, `sector`, `specs/`, `BACKLOG`) "
             ...
             f"{sum(1 for h in hyg.values() if h)} files. "
             + ("None remain: repository-internal pointers live in `**Provenance and references.**` "
                "paragraphs, which a Physlib PR may keep or drop. `scripts/export-physlib.sh --strict` passes.\n"
                if tot_h == 0 else
                "These are docstring cross-references to the consumers in this repository, not code; each is "
                "a line to reword or drop at PR time. `scripts/export-physlib.sh --strict` fails while the "
                "count is positive.\n"))
...
def main() -> None:
    ...
    if args.hits:
        for m in sorted(deps):
            for ln, name, line in hygiene_hits(read(mod_to_path(m))):
