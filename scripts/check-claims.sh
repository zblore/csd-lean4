...
EPISTEMIC_DOCS=("README.md" "EMPIRICAL.md" "AXIOMS.md" "specs/BACKLOG.md" \
                ...
                "specs/feynman-continuum-scoping.md")
...
# (13) CITATION.cff C1 anchor: the "cite tag" line must name the newest C1
...
if [ -n "$newest_c1" ] && [ -f CITATION.cff ]; then
  if grep -q "cite tag \`$newest_c1\`" CITATION.cff; then
    say_ok "CITATION.cff C1 anchor names the newest C1 tag ($newest_c1)"
  else
    say_fail "CITATION.cff C1 anchor is stale: newest C1 tag is $newest_c1 but the 'cite tag' line does not name it"
  fi
fi
