...
theorems carry the mathematics, and the other algorithms are still QM-side only. The two residues that stood here are
closed: the straddling-kernel bound behind the literal amplitude-estimation constant (`R-001`,
`amplitude_estimation_bhmt`, BHMT Theorem 11 with its `8/π²`) and the exponential-doubling schedule for unknown
amplitude (`R-002`, `qsearch_expected_cost`, BHMT Theorem 3's expected `O(1/√a)`). The interference picture most readers bring, Feynman's
...
BACKLOG #61, the circuit-level threshold theorem #62, the `Σ`-twin of the Steane recovery #53). `R-005`: the density of Clifford+T in the unitary group — eleven priced rows after two re-pricings, of which seven have landed (BACKLOG #68 the irrational rotation angle, #80 its axis–angle form, #70 the two-level decomposition, #71 the Gray-code sandwich, #83 the controlled-gate vocabulary, #84 the `z`-`y`-`z` Euler decomposition with the one-control circuit, #85 the control-count recursion — so a gate with any number of controls is now an exact product of `CNOT`s and single-qubit gates). What remains is #81 and then #73; #82 and #74 are deferred.
...
besides is the `T` gate itself and the fact
