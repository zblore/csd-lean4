...
| **C1 positive half: an explicit contextual model reproduces the singlet** | `singletContextualModel_table`, `singletContextualModel_reproduces` | `LF6/C1BellConsistency.lean` (added 2026-08-13, Q19) | On the concrete arena `(KSigma 4, kMuPsi)`: the **full `P_st` table at every context** (marginals `½`, correlations `−a·b`), hence the CHSH correlations. The table form is deliberate: a correlation-only witness could carry degenerate marginals, and "reproduces the singlet" would over-read it — the E-1 lesson applied in advance. |
| **C1 contextual capstone: existence ∧ obstruction** | `c1_singlet_contextual_capstone` | same | One statement: a measurable shared-context family **exists** that reproduces the singlet, and **no** global CHSH assignment is compatible with it. Contextual models do what non-contextual ones provably cannot; both halves witnessed. Incompatibility half still constrains only the four CHSH settings. Witness-arena restatement: `Tests/Witnesses/SingletBell.lean` `kMuPsi_singlet_contextual_model` (WS-H upgrade). |
