...
| The Kähler triple, pointwise (`g`, `ω`, `J`); and, at manifold level since 2026-09-07, the Fubini–Study form on `ℂℙⁿ` is closed and non-degenerate: a symplectic manifold | `fubiniStudy_pointwise_kahler_compatibility`, `fsForm_mextDeriv`, `fsForm_isSymplectic` | `Mathlib/Analysis/InnerProductSpace/KahlerForm`, `Mathlib/Geometry/Manifold/Instances/ProjectiveSpaceFubiniStudySymplectic` |
