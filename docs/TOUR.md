...
- **the de-isolation interface is populated by the dynamics** (2026-08-27,
  ...
  **externality pair** joined the dynamical bundle (`outcome_system_dependent`: before the
  ...
  the propagator's Hamiltonian generation is stated, not formalised (the manifold symplectic
  API exists since 2026-09-08, `HamiltonianVectorField.lean`; the propagator is not yet stated in
  it; `ShearWitness` item 1).
...
| The sector flow is Hamiltonian on the sector's own manifold (since 2026-09-09): for every Hermitian `H`, `exp(-itH)` on `ℂℙⁿ` is the integral curve of the Hamiltonian vector field of `-2⟨H⟩` for the Fubini–Study symplectic form, `⟨H⟩` conserved; the torus action is the diagonal case with the Born weights' moment map as its Hamiltonian, unique up to a constant, image the simplex | `schrodingerField_isHamiltonianVectorField`, `isMIntegralCurve_schrodingerUnitary_smul`, `torusField_isHamiltonianVectorField`, `eq_torusHamiltonian_of_nonneg_of_sum`, `range_momentMap` | `Mathlib/Geometry/Manifold/Instances/ProjectiveSpaceSchrodingerFlow`, `…ProjectiveSpaceMomentMap` |
...
| The Kähler triple, pointwise (`g`, `ω`, `J`); at manifold level `ℂℙⁿ` with the Fubini–Study form is a symplectic (2026-09-07) and Kähler (2026-09-10) manifold whose top power is `(4π)ⁿ μ_FS`; and the sector's Liouville measure IS that normalised top power, its unitary flows preserving it | `fubiniStudy_pointwise_kahler_compatibility`, `fsForm_isSymplectic`, `fsForm_isKahler`, `fsVolume_eq_smul_fubiniStudyMeasure`, `unitaryFlowSetup_isKahler_liouville` | `Mathlib/Analysis/InnerProductSpace/KahlerForm`, `Mathlib/Geometry/Manifold/Instances/ProjectiveSpaceFubiniStudySymplectic`, `LF4/SectorManifold` |
...
| **From the posits to the quantum-information layer** (2026-09-11/12): the density operator of a preparation on `Σ` IS the barycentre of its rays; channels are the environment marginals of `Σ`-flows lifting a unitary (the measurement flow yields the de-isolation channel, unconditionally); the second law, data processing and Landauer on `Σ`; concavity and the Holevo bound with no support hypothesis, the de-isolation channel's capacity = one bit; the three-qubit code on `Σ` end to end (error = a flow's leak into the environment, syndrome = a partition of `Σ`, recovery = one channel) | `preparationDensity_eq_barycenter`, `traceRight_barycenter_flow`, `measurementFlow_traceRight_barycenter_unitSection`, `vonNeumannEntropy_le_pinching_flow`, `landauer_flow`, `deisolationChannel_holevoCapacity`, `registerFlow_recovery` | `LF2/PreparationBarycenter`, `LF2/FlowChannel`, `LF6/MeasurementFlowChannel`, `Thermo/SigmaSecondLaw`, `LF6/DeisolationCapacity`, `Empirical/CSD/QEC/RegisterFlow` |
| The flow of a `C¹` field on a compact manifold exists, obeys the group law and is jointly continuous (2026-09-11/12; Mathlib has no flows on manifolds) | `integralFlow_add`, `continuous_integralFlow` | `Mathlib/Geometry/Manifold/IntegralCurve/GlobalFlow`, `FlowContinuity` |
