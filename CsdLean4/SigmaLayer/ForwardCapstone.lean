...
It is named `product_choiceA_forward_capstone`, not `finiteQM_complete`. It does NOT claim: general
...
The capstone `product_choiceA_forward_capstone` has NO hypotheses beyond `(H, hH, p₀)` (a Hermitian
...
/-- **The product-model Choice A forward capstone.** For the many-to-one product sector with the
...
This is the forward delivery of the projective dynamics and the measure bridge from the Choice A ontic
sector for this concrete model, combining only proved components. -/
theorem product_choiceA_forward_capstone :
