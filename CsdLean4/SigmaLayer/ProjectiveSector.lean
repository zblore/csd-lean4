...
structure ProjectiveSector (N : ℕ) {Sigma : Type u} [MeasurableSpace Sigma]
...
theorem projectiveLaw_apply (Q : ProjectiveSector N D) (mu : Measure Sigma)
...
def kahlerProjectiveSector {N : ℕ} (K : CSD.LF4.KahlerOnticSetup N)
    {D : ConstraintDynamics K.Sigma} : ProjectiveSector N D where
  pi := K.pi
