...
/-!
# FND-2: a genuine `Φ ≠ id` on the concrete ENTANGLED (singlet) sector
...
`Σ = ℂℙ³ × T²` (`LF4/SingletKahler.lean`), FND-2. The singlet preparation
...
Fubini–Study typicality is still posited; that is FND-1). Foundational-triple-only
...
/-- **FND-2: the singlet preparation on the `Φ ≠ id` sector.** The concrete
...
via `kFlow_ne_id`. This is the FND-2 headline: the concrete ENTANGLED sector now
...
/-- **FND-2 capstone: the singlet frequencies survive the sector's own flow.** For
