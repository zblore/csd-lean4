...
isometry is the (index-generic) staged `Projectivization.inner_toEuclideanLin_unitary`. -/
noncomputable def FloquetEvolution.ofUnitaryMatrix
  ...
  step :=
    { toLinearEquiv := LinearEquiv.ofLinearMap
        ...
        (by
          rw [← Matrix.toLpLin_mul_same 2 (U.val : Matrix n n ℂ) (star U.val),
              show (U.val : Matrix n n ℂ) * star U.val = 1 from
                Unitary.coe_mul_star_self U,
              Matrix.toLpLin_one 2])
        (by
          rw [← Matrix.toLpLin_mul_same 2 (star U.val : Matrix n n ℂ) U.val,
              show (star U.val : Matrix n n ℂ) * U.val = 1 from
      ...
      norm_map' := fun v => by
        ...
        exact congrArg RCLike.re (Projectivization.inner_toEuclideanLin_unitary U v v) }

@[simp] lemma FloquetEvolution.ofUnitaryMatrix_step_apply
    {n : Type*} [Fintype n] [DecidableEq n]
    (U : Matrix.unitaryGroup n ℂ) (ψ : EuclideanSpace ℂ n) :
    (FloquetEvolution.ofUnitaryMatrix U).step ψ = Matrix.toEuclideanLin U.val ψ :=
  rfl
