...
A point of `ℙ ℂ (EuclideanSpace ℂ (Fin N))` is a ray `[z]`, and the squared moduli of its
...
noncomputable def momentMap (p : ℙ ℂ (EuclideanSpace ℂ (Fin N))) (i : Fin N) : ℝ :=
  ‖p.rep i‖ ^ 2 / ‖p.rep‖ ^ 2

/-- Each moment coordinate is nonnegative. -/
lemma momentMap_nonneg (p : ℙ ℂ (EuclideanSpace ℂ (Fin N))) (i : Fin N) :
    0 ≤ momentMap p i :=
  div_nonneg (sq_nonneg _) (sq_nonneg _)

/-- The moment coordinates sum to one: the image lands in the standard simplex. -/
lemma momentMap_sum_eq_one (p : ℙ ℂ (EuclideanSpace ℂ (Fin N))) :
...
lemma momentMap_le_one (p : ℙ ℂ (EuclideanSpace ℂ (Fin N))) (i : Fin N) :
    momentMap p i ≤ 1 := by
...
modulus, the moment map of `[ψ]` is the uniform weight `1/N` in every coordinate; the
...
theorem continuous_momentMap (i : Fin N) :
    Continuous (fun p : ℙ ℂ (EuclideanSpace ℂ (Fin N)) => momentMap p i) := by
  rw [continuous_iff_continuous_comp_mk']
  have hcomp : ((fun p : ℙ ℂ (EuclideanSpace ℂ (Fin N)) => momentMap p i) ∘ (mk' ℂ))
...
theorem measurable_momentMap (i : Fin N) :
    Measurable (fun p : ℙ ℂ (EuclideanSpace ℂ (Fin N)) => momentMap p i) :=
