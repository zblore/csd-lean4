...
it is a proved circuit-to-circuit transform. The abstract state is deliberately conservative (a written
wire becomes `Unknown` unless the write is an `X`/`swap`), which is sound and already captures the main
...
theorem foldGate_denote {α : Abs m} {s : State m} (h : absAgree α s) (g : Gate m) :
  ...
  | swap i j => rfl
  | CCX c₁ c₂ t =>
    show denote (foldGate α (.CCX c₁ c₂ t)) s = denoteGate (.CCX c₁ c₂ t) s
    unfold foldGate
    split_ifs with hdeg h0 h1 h2
    · rw [denote_nil, denoteGate, if_pos hdeg]
    · rw [denote_nil, denoteGate, if_neg hdeg]
    ...
    · -- α c₁ = some true → fold to CX c₂ t
      have hc₁ : s c₁ = true := h c₁ true h1
      have hne2 : ¬ (c₂ = t) := fun e => hdeg (Or.inr e.symm)
      show denote [Gate.CX c₂ t] s = denoteGate (.CCX c₁ c₂ t) s
      rw [show denote [Gate.CX c₂ t] s = denoteGate (.CX c₂ t) s from rfl,
        denoteGate, denoteGate, if_neg hne2, if_neg hdeg, hc₁, Bool.true_and, Bool.xor_comm]
    · -- α c₂ = some true → fold to CX c₁ t
      have hc₂ : s c₂ = true := h c₂ true h2
      have hne1 : ¬ (c₁ = t) := fun e => hdeg (Or.inl e.symm)
      show denote [Gate.CX c₁ t] s = denoteGate (.CCX c₁ c₂ t) s
      rw [show denote [Gate.CX c₁ t] s = denoteGate (.CX c₁ t) s from rfl,
        denoteGate, denoteGate, if_neg hne1, if_neg hdeg, hc₂, Bool.and_true, Bool.xor_comm]
    · rfl
...
def stepAbs (α : Abs m) : Gate m → Abs m
  | .X i => Function.update α i ((α i).map not)
...
theorem stepAbs_agree {α : Abs m} {s : State m} (h : absAgree α s) (g : Gate m) :
  ...
  cases g with
  | X j =>
    simp only [stepAbs, denoteGate, Function.update_apply] at hib ⊢
    by_cases hij : i = j
    · subst hij
      simp only [if_true] at hib ⊢
      cases hα : α i with
      | none => rw [hα] at hib; simp at hib
      | some x => rw [hα] at hib; simp only [Option.map_some'] at hib
                  rw [h i x hα]; simp only [Option.some.injEq] at hib; omega
    · simp only [if_neg hij] at hib ⊢; exact h i b hib
  | CX c t =>
    simp only [stepAbs, denoteGate, Function.update_apply] at hib ⊢
    by_cases hit : i = t
    · subst hit; simp at hib
    · simp only [if_neg hit] at hib ⊢
      rcases eq_or_ne c t with rfl | hct
      · simp only [if_pos rfl]; exact h i b hib
      · simp only [if_neg hct, if_neg hit]; exact h i b hib
  | CCX c₁ c₂ t =>
    simp only [stepAbs, denoteGate, Function.update_apply] at hib ⊢
    by_cases hit : i = t
    · subst hit; simp at hib
    · simp only [if_neg hit] at hib ⊢
      ...
      · simp only [if_pos hdeg]; exact h i b hib
      · simp only [if_neg hdeg, if_neg hit]; exact h i b hib
  | swap p q =>
    simp only [stepAbs, denoteGate, Function.comp_apply] at hib ⊢
    by_cases hip : i = p
    · subst hip; simp only [if_pos rfl, Equiv.swap_apply_left]; exact h q b hib
    · by_cases hiq : i = q
      · subst hiq; simp only [if_neg hip, if_pos rfl, Equiv.swap_apply_right]; exact h p b hib
      · simp only [if_neg hip, if_neg hiq, Equiv.swap_apply_of_ne_of_ne hip hiq]; exact h i b hib
...
theorem demoAbs_agree (s : State 3) (h0 : s 0 = false) : absAgree demoAbs s := by
  ...
  by_cases hi : i = 0
  · subst hi
    simp only [demoAbs, if_pos rfl, Option.some.injEq] at hib
    rw [h0, hib]
