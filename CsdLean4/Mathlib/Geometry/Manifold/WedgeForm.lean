...
With the flat wedge bounded and
...
* ★★ `DifferentialForm.wedgePow α k` — **the `k`-th exterior power of a real 2-form**, a
  ...
  commutes with the power), `wedgePow_smul` (homogeneity) and ★ `localRep_wedgePow` (the local
  representative of the power is
...
def wedgeFamily (B : F →L[ℝ] G →L[ℝ] H)
    (α : ∀ x : M, TangentSpace (𝓘(ℝ, E)) x [⋀^ιa]→L[ℝ] Bundle.Trivial M F x)
    (β : ∀ x : M, TangentSpace (𝓘(ℝ, E)) x [⋀^ιb]→L[ℝ] Bundle.Trivial M G x)
    (x : M) :
    TangentSpace (𝓘(ℝ, E)) x [⋀^ιa ⊕ ιb]→L[ℝ] Bundle.Trivial M H x :=
...
theorem trivializationAt_wedgeFamily_snd (B : F →L[ℝ] G →L[ℝ] H)
    ...
    (trivializationAt (E [⋀^ιa ⊕ ιb]→L[ℝ] H)
      (fun x : M => TangentSpace (𝓘(ℝ, E)) x [⋀^ιa ⊕ ιb]→L[ℝ] Bundle.Trivial M H x)
...
theorem contMDiff_wedgeFamily (B : F →L[ℝ] G →L[ℝ] H)
    ...
    {β : ∀ x : M, TangentSpace (𝓘(ℝ, E)) x [⋀^ιb]→L[ℝ] Bundle.Trivial M G x}
    (hα : ContMDiff (𝓘(ℝ, E))
      ((𝓘(ℝ, E)).prod (𝓘(ℝ, E [⋀^ιa]→L[ℝ] F))) ∞
      (fun x : M => TotalSpace.mk' (E [⋀^ιa]→L[ℝ] F) x (α x)))
    (hβ : ContMDiff (𝓘(ℝ, E))
      ((𝓘(ℝ, E)).prod (𝓘(ℝ, E [⋀^ιb]→L[ℝ] G))) ∞
      (fun x : M => TotalSpace.mk' (E [⋀^ιb]→L[ℝ] G) x (β x))) :
    ContMDiff (𝓘(ℝ, E))
      ((𝓘(ℝ, E)).prod (𝓘(ℝ, E [⋀^ιa ⊕ ιb]→L[ℝ] H))) ∞
...
def wedge (B : F →L[ℝ] G →L[ℝ] H) (α : DifferentialForm (𝓘(ℝ, E)) M ∞ ιa F)
    (β : DifferentialForm (𝓘(ℝ, E)) M ∞ ιb G) :
    DifferentialForm (𝓘(ℝ, E)) M ∞ (ιa ⊕ ιb) H :=
...
@[simp] theorem wedge_apply (B : F →L[ℝ] G →L[ℝ] H)
    (α : DifferentialForm (𝓘(ℝ, E)) M ∞ ιa F)
    (β : DifferentialForm (𝓘(ℝ, E)) M ∞ ιb G) (x : M) :
...
def domDomCongrFamily (σ : ι ≃ ι')
    (α : ∀ x : M, TangentSpace (𝓘(ℝ, E)) x [⋀^ι]→L[ℝ] Bundle.Trivial M F x)
    (x : M) : TangentSpace (𝓘(ℝ, E)) x [⋀^ι']→L[ℝ] Bundle.Trivial M F x :=
  (ContinuousAlternatingMap.domDomCongr σ (toFlat (α x)) : E [⋀^ι']→L[ℝ] F)

omit [IsManifold (𝓘(ℝ, E)) ∞ M] [Fintype ι] [DecidableEq ι] [Fintype ι']
  [DecidableEq ι'] in
...
theorem trivializationAt_domDomCongrFamily_snd (σ : ι ≃ ι')
    ...
    (trivializationAt (E [⋀^ι']→L[ℝ] F)
      (fun x : M => TangentSpace (𝓘(ℝ, E)) x [⋀^ι']→L[ℝ] Bundle.Trivial M F x)
...
theorem contMDiff_domDomCongrFamily (σ : ι ≃ ι')
  ...
  have h1 : ContMDiffAt (𝓘(ℝ, E)) (𝓘(ℝ, E [⋀^ι']→L[ℝ] F)) ∞
...
def domDomCongr (σ : ι ≃ ι') (α : DifferentialForm (𝓘(ℝ, E)) M ∞ ι F) :
    DifferentialForm (𝓘(ℝ, E)) M ∞ ι' F :=
  ⟨domDomCongrFamily σ (fun x => α x), contMDiff_domDomCongrFamily σ α.contMDiff_toFun⟩

omit [DecidableEq ι'] in
@[simp] theorem domDomCongr_apply (σ : ι ≃ ι')
    (α : DifferentialForm (𝓘(ℝ, E)) M ∞ ι F) (x : M) :
...
def constZeroFamily [IsEmpty ι] (c : F) (x : M) :
    TangentSpace (𝓘(ℝ, E)) x [⋀^ι]→L[ℝ] Bundle.Trivial M F x :=
...
def constZero [IsEmpty ι] (c : F) : DifferentialForm (𝓘(ℝ, E)) M ∞ ι F :=
...
def wedgePow (α₂ : DifferentialForm (𝓘(ℝ, E)) M ∞ (Fin 2) ℝ) :
    (k : ℕ) → DifferentialForm (𝓘(ℝ, E)) M ∞ (Fin (2 * k)) ℝ
...
@[simp] theorem wedgePow_succ (α₂ : DifferentialForm (𝓘(ℝ, E)) M ∞ (Fin 2) ℝ)
    (k : ℕ) :
    wedgePow α₂ (k + 1) = domDomCongr (powEquiv k) (wedge (ContinuousLinearMap.mul ℝ ℝ) (wedgePow α₂
        k) α₂) :=
  rfl
...
representative. -/
theorem localRep_wedgePow (α : DifferentialForm (𝓘(ℝ, E)) M ∞ (Fin 2) ℝ)
    (x₀ : M) {w : E} (hw : w ∈ (chartAt E x₀).target) :
