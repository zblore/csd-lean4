...
Feynman–Kac (FC-4). One dimension, as the Brownian motion of the pin.
...
/-- **Freezing an independent variable**: for independent `X : Ω → ℝ` and `Y : Ω → 𝒴` and a bounded
measurable `Ψ`, `E[Ψ (X, Y)] = E_ω[E_ω'[Ψ (X ω, Y ω')]]`. -/
theorem integral_prod_of_indepFun [IsProbabilityMeasure P] {𝒴 : Type*} [MeasurableSpace 𝒴]
    {X : Ω → ℝ} {Y : Ω → 𝒴} (hXY : IndepFun X Y P) (hX : AEMeasurable X P) (hY : AEMeasurable Y P)
    {Ψ : ℝ × 𝒴 → ℂ} (hΨ : Measurable Ψ) {C : ℝ} (hC : ∀ p, ‖Ψ p‖ ≤ C) :
...
noncomputable def slicedWiener (B : ℝ≥0 → Ω → ℝ) (h : ℝ≥0) (g f : ℝ → ℂ) (n : ℕ) (x : ℝ) : ℂ :=
  ∫ ω, (∏ k ∈ Finset.range n, g (x + B (((k + 1 : ℕ) : ℝ≥0) * h) ω)) * f (x + B ((n : ℝ≥0) * h) ω) ∂P

theorem slicedWiener_zero [IsProbabilityMeasure P] {B : ℝ≥0 → Ω → ℝ} (hB : IsPreBrownianReal B P)
    (h : ℝ≥0) (g f : ℝ → ℂ) (x : ℝ) : slicedWiener P B h g f 0 x = f x := by
  rw [slicedWiener]
...
path. -/
theorem measurable_pathFunctional {g f : ℝ → ℂ} (hg : Measurable g) (hf : Measurable f) (h : ℝ≥0)
    (n : ℕ) :
    Measurable fun p : ℝ × (ℝ≥0 → ℝ) =>
...
theorem norm_pathFunctional_le {g f : ℝ → ℂ} {Cg Cf : ℝ} (hCg : ∀ x, ‖g x‖ ≤ Cg)
    (hCf : ∀ x, ‖f x‖ ≤ Cf) (h : ℝ≥0) (n : ℕ) (p : ℝ × (ℝ≥0 → ℝ)) :
...
theorem measurable_slicedWiener [IsProbabilityMeasure P] {B : ℝ≥0 → Ω → ℝ}
    (hB : ∀ t, Measurable (B t)) (h : ℝ≥0) {g f : ℝ → ℂ} (hg : Measurable g) (hf : Measurable f)
...
theorem slicedWiener_succ [IsProbabilityMeasure P] {B : ℝ≥0 → Ω → ℝ} (hB : IsPreBrownianReal B P)
    (hBm : ∀ t, Measurable (B t)) (h : ℝ≥0) {g f : ℝ → ℂ} (hg : Measurable g)
    (hf : Measurable f) {Cg Cf : ℝ} (hCg : ∀ x, ‖g x‖ ≤ Cg) (hCf : ∀ x, ‖f x‖ ≤ Cf) (n : ℕ) (x : ℝ) :
  ...
  set B' : ℝ≥0 → Ω → ℝ := fun t ω => B (h + t) ω - B h ω with hB'
  have hind : IndepFun (B h) (fun ω t => B' t ω) P := by
    have h1 := (hB.indepFun_shift h).comp measurable_id
      (measurable_pi_apply (⟨h, Set.mem_Iic.mpr le_rfl⟩ : Set.Iic h))
    exact h1.symm
  set Ψ : ℝ × (ℝ≥0 → ℝ) → ℂ := fun p =>
    g (x + p.1) * ((∏ k ∈ Finset.range n, g (x + p.1 + p.2 (((k + 1 : ℕ) : ℝ≥0) * h)))
      * f (x + p.1 + p.2 ((n : ℝ≥0) * h))) with hΨ
  have hΨm : Measurable Ψ := by
    refine (hg.comp (measurable_const.add measurable_fst)).mul ?_
    have := measurable_pathFunctional hg hf h n
    exact this.comp ((measurable_const.add measurable_fst).prodMk measurable_snd)
  have hCg0 : 0 ≤ Cg := le_trans (norm_nonneg _) (hCg 0)
  ...
  have hL : slicedWiener P B h g f (n + 1) x = ∫ ω, Ψ (B h ω, fun t => B' t ω) ∂P := by
    ...
    ring
  rw [hL, integral_prod_of_indepFun hind (hB.aemeasurable h)
    (measurable_pi_iff.mpr hB'm).aemeasurable hΨm hΨb]
  -- `E[Φ(B_h)] = ∫ Φ dγ_h`
  have hlaw := hB.hasLaw_eval h
  set Φ : ℝ → ℂ := fun z => g (x + z) * slicedWiener P B' h g f n (x + z) with hΦdef
  ...
  calc ∫ ω, (∫ ω', Ψ (B h ω, fun t => B' t ω') ∂P) ∂P
    ...
    _ = ∫ z, Φ z ∂gaussianReal 0 h := hlaw.integral_comp hΦm.aestronglyMeasurable
...
noncomputable def stepOp (h : ℝ≥0) (gL : Lp ℂ ∞ (volume : Measure ℝ)) : L2 →L[ℂ] L2 :=
  heatSemigroup (h : ℝ) * potential gL

theorem stepOp_apply (h : ℝ≥0) (gL : Lp ℂ ∞ (volume : Measure ℝ)) (v : L2) :
    stepOp h gL v = heatSemigroup (h : ℝ) (potential gL v) :=
  mul_apply_eq_comp _ _ _

/-- An almost-everywhere identity on `ℝ` holds `γ_h`-almost everywhere along every translate. -/
theorem ae_gaussian_of_ae {h : ℝ≥0} (hh : 0 < h) {u v : ℝ → ℂ} (huv : u =ᵐ[volume] v) (x : ℝ) :
    (fun y => u (x + y)) =ᵐ[gaussian (h : ℝ)] fun y => v (x + y) := by
  have h1 := (measurePreserving_add_left (volume : Measure ℝ) x).quasiMeasurePreserving.ae_eq_comp huv
  rw [gaussian, Real.toNNReal_coe]
  exact (gaussianReal_absolutelyContinuous 0 hh.ne').ae_eq h1
...
theorem pow_stepOp_apply_ae_eq_slicedWiener [IsProbabilityMeasure P] {h : ℝ≥0} (hh : 0 < h)
    {g f : ℝ → ℂ} (hg : Measurable g) (hf : Measurable f) {Cg Cf : ℝ} (hCg : ∀ x, ‖g x‖ ≤ Cg)
    (hCf : ∀ x, ‖f x‖ ≤ Cf) (hf2 : MemLp f 2 volume) {gL : Lp ℂ ∞ (volume : Measure ℝ)}
    (hgL : gL =ᵐ[volume] g) :
    ∀ (n : ℕ) (B : ℝ≥0 → Ω → ℝ), IsPreBrownianReal B P → (∀ t, Measurable (B t)) →
  ...
  | succ n ih =>
    intro B hB hBm
    set B' : ℝ≥0 → Ω → ℝ := fun t ω => B (h + t) ω - B h ω with hB'
    have hB'pre : IsPreBrownianReal B' P := hB.shift h
    have hB'm : ∀ t, Measurable (B' t) := fun t => (hBm _).sub (hBm _)
    have ihB' := ih B' hB'pre hB'm
    set u : L2 := potential gL ((stepOp h gL ^ n) (hf2.toLp f)) with hu
    have hueq : (u : ℝ → ℂ) =ᵐ[volume] fun z => g z * slicedWiener P B' h g f n z := by
