...
removes the boundedness (BACKLOG #42, FC-4′): for every `g ∈ L²(ℝᵈ)`,
...
⚠️ Conditional on a Brownian motion in `ℝᵈ` (`hB : IsBrownianVec B P`), bounded continuous
...
noncomputable def weight (V : E → ℝ) (B : ℝ≥0 → Ω → E) (t : ℝ≥0) (x : E) (ω : Ω) : ℂ :=
  Complex.exp (-(((∫ s in (0 : ℝ)..(t : ℝ), V (x + B (Real.toNNReal s) ω)) : ℝ) : ℂ))

/-- The Feynman–Kac functional `E[exp (−∫₀ᵗ V (x + B_s) ds) · f (x + B_t)]`. -/
noncomputable def fkFunctional (V : E → ℝ) (P : Measure Ω) (B : ℝ≥0 → Ω → E) (t : ℝ≥0)
    (f : E → ℂ) (x : E) : ℂ :=
  ∫ ω, weight V B t x ω * f (x + B t ω) ∂P

variable {V : EuclideanSpace ℝ ι → ℝ} {CV : ℝ} (hCV : ∀ x, |V x| ≤ CV)

omit [MeasurableSpace Ω] [Fintype ι] in
include hCV in
theorem norm_weight_le (B : ℝ≥0 → Ω → E) (t : ℝ≥0) (x : E) (ω : Ω) :
...
theorem aestronglyMeasurable_pathIntegral (hVc : Continuous V) {B : ℝ≥0 → Ω → E}
    (hB : IsBrownianVec B P) (hBm : ∀ t, Measurable (B t)) {t : ℝ≥0} (ht : 0 < t) (x : E) :
...
theorem aestronglyMeasurable_weight (hVc : Continuous V) {B : ℝ≥0 → Ω → E}
    (hB : IsBrownianVec B P) (hBm : ∀ t, Measurable (B t)) {t : ℝ≥0} (ht : 0 < t) (x : E) :
...
include hCV in
theorem integrable_weight_mul (hVc : Continuous V) {B : ℝ≥0 → Ω → E} (hB : IsBrownianVec B P)
    (hBm : ∀ t, Measurable (B t)) {t : ℝ≥0} (ht : 0 < t) {f : E → ℂ} {x : E}
...
a Gaussian law. -/
theorem fkFunctional_congr_ae {B : ℝ≥0 → Ω → E} (hB : IsBrownianVec B P)
    (hBm : ∀ t, Measurable (B t)) {t : ℝ≥0} (ht : 0 < t) {f₁ f₂ : E → ℂ}
    (h : f₁ =ᵐ[volume] f₂) (x : E) : fkFunctional V P B t f₁ x = fkFunctional V P B t f₂ x := by
  refine integral_congr_ae ?_
  have h1 : ∀ᵐ y ∂(volume : Measure E), f₁ (x + y) = f₂ (x + y) :=
    (measurePreserving_add_left volume x).quasiMeasurePreserving.ae_eq h
  have h2 : ∀ᵐ y ∂(gaussianVec ι t), f₁ (x + y) = f₂ (x + y) :=
    (gaussianVec_absolutelyContinuous ht.ne').ae_eq h1
  rw [← (hB.toIsPreBrownianVec.hasLaw_eval hBm t).map_eq] at h2
...
theorem ae_integrable_shift {B : ℝ≥0 → Ω → E} (hB : IsBrownianVec B P)
    (hBm : ∀ t, Measurable (B t)) (t : ℝ≥0) (g : L2) :
    ∀ᵐ x ∂(volume : Measure E), Integrable (fun ω => g (x + B t ω)) P := by
  ...
  rw [gaussian, Real.toNNReal_coe, ← (hB.toIsPreBrownianVec.hasLaw_eval hBm t).map_eq] at hx
...
theorem coeFn_absL (g : L2) : (absL g : E → ℂ) =ᵐ[volume] fun x => ((‖g x‖ : ℝ) : ℂ) :=
  MemLp.coeFn_toLp _
...
theorem heatConv_congr_ae {t : ℝ} (ht : 0 < t) {f₁ f₂ : E → ℂ} (h : f₁ =ᵐ[volume] f₂) (x : E) :
    heatConv t f₁ x = heatConv t f₂ x := by
  simp only [heatConv]
  refine integral_congr_ae ?_
  have h1 : ∀ᵐ y ∂(volume : Measure E), f₁ (x + y) = f₂ (x + y) :=
    (measurePreserving_add_left volume x).quasiMeasurePreserving.ae_eq h
  exact (gaussian_absolutelyContinuous ht).ae_eq h1

/-- ★ **The averaged modulus is the heat semigroup of `|g|`**: `E‖g (x + B_t)‖ = (P_t |g|)(x)` for
almost every `x`. -/
theorem absConv_ae_eq {B : ℝ≥0 → Ω → E} (hB : IsBrownianVec B P) (hBm : ∀ t, Measurable (B t))
    {t : ℝ≥0} (ht : 0 < t) (g : L2) :
  ...
  have h1 : ∀ x, ((∫ ω, ‖g (x + B t ω)‖ ∂P : ℝ) : ℂ)
    ...
    intro x
    rw [heatConv_eq_integral_brownian hB.toIsPreBrownianVec hBm htR
...
theorem integrableOn_absConv {B : ℝ≥0 → Ω → E} (hB : IsBrownianVec B P)
    (hBm : ∀ t, Measurable (B t)) {t : ℝ≥0} (ht : 0 < t) (g : L2) {A : Set E} (hμA : volume A < ∞) :
    IntegrableOn (fun x => ∫ ω, ‖g (x + B t ω)‖ ∂P) A volume := by
  have : IsFiniteMeasure ((volume : Measure E).restrict A) :=
  ...
  filter_upwards [ae_restrict_of_ae (absConv_ae_eq hB hBm ht g)] with x hx
...
theorem setIntegral_absConv_le {B : ℝ≥0 → Ω → E} (hB : IsBrownianVec B P)
    (hBm : ∀ t, Measurable (B t)) {t : ℝ≥0} (ht : 0 < t) (g : L2) {A : Set E} (hA : MeasurableSet A)
    (hμA : volume A < ∞) :
  ...
  have hre : ∫ x in A, ∫ ω, ‖g (x + B t ω)‖ ∂P
    ...
    rw [← setIntegral_congr_ae hA ((absConv_ae_eq hB hBm ht g).mono fun x hx _ => hx),
...
include hCV in
theorem norm_fkFunctional_le {B : ℝ≥0 → Ω → E} (hB : IsBrownianVec B P)
    (hBm : ∀ t, Measurable (B t)) (t : ℝ≥0) (g : L2) :
    ∀ᵐ x ∂(volume : Measure E), ‖fkFunctional V P B t g x‖
...
`‖∫_A fk g‖ ≤ e^{t‖V‖} ‖1_A‖ ‖g‖`. -/
theorem norm_setIntegral_fkFunctional_le {B : ℝ≥0 → Ω → E}
    (hB : IsBrownianVec B P) (hBm : ∀ t, Measurable (B t)) {t : ℝ≥0} (ht : 0 < t) (g : L2)
    {A : Set E} (hA : MeasurableSet A) (hμA : volume A < ∞) :
  ...
  calc ∫ x in A, ‖fkFunctional V P B t g x‖
      ≤ ∫ x in A, Real.exp ((t : ℝ) * CV) * ∫ ω, ‖g (x + B t ω)‖ ∂P :=
        integral_mono_of_nonneg (Eventually.of_forall fun x => norm_nonneg _)
          ((integrableOn_absConv hB hBm ht g hμA).const_mul _)
    ...
    _ ≤ Real.exp ((t : ℝ) * CV) * (‖indicatorConstLp 2 hA hμA.ne (1 : ℂ)‖ * ‖g‖) := by
        gcongr
        exact setIntegral_absConv_le hB hBm ht g hA hμA
...
theorem aestronglyMeasurable_fkFunctional [IsProbabilityMeasure P] (hVc : Continuous V)
    {B : ℝ≥0 → Ω → E} (hB : IsBrownianVec B P) (hBm : ∀ t, Measurable (B t)) {t : ℝ≥0}
    (ht : 0 < t) (g : L2) :
    AEStronglyMeasurable (fkFunctional V P B t g) (volume : Measure E) := by
...
theorem integrableOn_fkFunctional [IsProbabilityMeasure P] (hVc : Continuous V)
    {B : ℝ≥0 → Ω → E} (hB : IsBrownianVec B P) (hBm : ∀ t, Measurable (B t)) {t : ℝ≥0}
    (ht : 0 < t) (g : L2) {A : Set E} (hμA : volume A < ∞) :
    IntegrableOn (fkFunctional V P B t g) A volume :=
  ((integrableOn_absConv hB hBm ht g hμA).const_mul _).mono'
...
theorem feynmanKac_simpleFunc [IsProbabilityMeasure P] {B : ℝ≥0 → Ω → E} (hB : IsBrownianVec B P)
    (hBm : ∀ t, Measurable (B t)) (hVc : Continuous V) {VL : Lp ℂ ∞ (volume : Measure E)}
    (hVL : (VL : E → ℂ) =ᵐ[volume] fun x => (V x : ℂ)) {t : ℝ≥0} (ht : 0 < t)
    (s : Lp.simpleFunc ℂ 2 (volume : Measure E)) :
    (perturbedHeat VL (t : ℝ) (s : L2) : E → ℂ) =ᵐ[volume] fkFunctional V P B t (s : L2) := by
...
theorem feynmanKac_Lp [IsProbabilityMeasure P] {B : ℝ≥0 → Ω → E} (hB : IsBrownianVec B P)
    (hBm : ∀ t, Measurable (B t)) (hVc : Continuous V) {VL : Lp ℂ ∞ (volume : Measure E)}
    (hVL : (VL : E → ℂ) =ᵐ[volume] fun x => (V x : ℂ)) (g : L2) {t : ℝ≥0} (ht : 0 < t) :
    (perturbedHeat VL (t : ℝ) g : E → ℂ) =ᵐ[volume] fun x =>
      ∫ ω, Complex.exp (-(((∫ s in (0 : ℝ)..(t : ℝ), V (x + B (Real.toNNReal s) ω)) : ℝ) : ℂ))
        * g (x + B t ω) ∂P := by
  show (perturbedHeat VL (t : ℝ) g : E → ℂ) =ᵐ[volume] fkFunctional V P B t g
  ...
  · have : IsFiniteMeasure ((volume : Measure E).restrict A) :=
    ...
    have key : ∀ ε > 0, ‖(∫ x in A, perturbedHeat VL (t : ℝ) g x)
      ...
      intro ε hε
      obtain ⟨s, hs⟩ := (Lp.simpleFunc.denseRange (p := 2) (μ := (volume : Measure E))
